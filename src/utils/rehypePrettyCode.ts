...
const normalizeLanguage = (lang?: string): string => {
  ...
  return bundledLanguages[mapped as keyof typeof bundledLanguages]
    ? mapped
    : "text";
};
...
const ensureLineText = (line: Element) => {
  const hasTextChild = line.children.some(
    (child) => child.type === "text" || child.type === "element",
  );
  if (!hasTextChild) {
...
const rehypePrettyCode: Plugin<[], Root> = () => {
  return async (tree: Root): Promise<void> => {
    ...
    visit(tree, "element", (node: Element, index, parent) => {
      ...
      const raw =
        (codeNode.properties as any)?.dataRawCode ||
        codeNode.children.map((child: any) => child.value || "").join("");
      const langFromData = (codeNode.properties as any)?.dataLang as
        | string
        | undefined;
      ...
      const highlightLines = parseHighlightSet(
        (codeNode.properties as any)?.dataHighlightLines as string | undefined,
      );
      const showLineNumbers =
        (codeNode.properties as any)?.dataLineNumbers !== "false";
      const title = (codeNode.properties as any)?.dataTitle as
        | string
        | undefined;
