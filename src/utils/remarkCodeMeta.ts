...
const noLineNumbersPattern = /(?:no-?line-?numbers|line-?numbers\s*=\s*false|nolines)/i;
...
const cleanMeta = (meta?: string): { meta: string; title?: string; showLineNumbers: boolean; highlights: number[] } => {
...
const remarkCodeMeta: Plugin<[], Root> = () => {
  return (tree) => {
    visit(tree, 'code', (node) => {
      ...
      const { meta, title, showLineNumbers, highlights } = cleanMeta(node.meta || '');
      ...
      (node.data as any).hProperties = {
        ...
        dataMeta: meta,
        dataTitle: title,
        dataHighlightLines: highlights.length ? highlights.join(',') : undefined,
