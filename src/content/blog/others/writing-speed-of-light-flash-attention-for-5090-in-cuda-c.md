...
imported_at: '2025-12-30T02:36:59.718Z'
source:
...
请注意，虽然我在这些实现中仅使用Ampere功能（sm120支持`cp.async.bulk`即TMA，但我这里不使用），我的实现可能无法在早期GPU上高效运行。由于新硬件的改进，您可能需要更多技巧才能在旧GPU上达到光速，例如流水线共享内存到寄存器内存的数据移动。
...
技术上，如果输入是BF16，一些计算应保持在FP32，尤其是softmax。但为简洁起见，我们省略它们。
...
暗示`DIM`很小，这样我们可以在内核的整个持续时间内将`tile_Q`保存在寄存器内存中。这就是为什么几乎所有现代模型都使用`head_dim=128`的原因。当然也有例外，比如[MLA](https://arxiv.org/abs/2405.04434)，它使用`head_dim=576`用于Q和K，以及`head_dim=512`用于V。谈到这个，我应该有一天研究[FlashMLA](https://github.com/deepseek-ai/FlashMLA)。

在线softmax相当棘手，难以解释，所以让我们延迟那部分的解释。在高层次上，您只需要知道在线softmax会将`tile_S`转换为`tile_P`，并重新缩放`tile_O`。
...
我想首先专注于正确实现算法，因此省略了更复杂的技巧，如共享内存交换和流水线。这减少了错误的可能性，我们稍后将重新审视它们以进行性能优化。
...
![Global to Shared data transfer](/images/others/others-fa-5090/001-d8f57fc4.svg)

从全局内存到共享内存的2D块复制。

我们将使用内联汇编编写`cp.async.cg.shared.global`。此PTX执行16字节传输，或8个BF16元素（`num_elems = 16 / sizeof(nv_bfloat16)`），对于每个CUDA线程。为确保合并内存访问，连续线程将负责连续的8xBF16组。

![Coalesced memory access](/images/others/others-fa-5090/002-b5fea70a.svg)

连续线程负责连续的8xBF16组。

注意：

- 循环`for (int iter = 0; iter < num_iters; iter++)`这样编写，以便编译器（`nvcc`）可以完全展开循环。`num_iters`在编译时已知（由`constexpr`保证）。如果我们在循环中混合`tid`，这对编译器来说是“动态”变量，循环无法展开，即使我们知道变量的某些约束，即`tid < TB_SIZE`。
...
在进行全局到共享内存的数据传输时，我们以线程块瓦片和单个CUDA线程为单位思考。对于共享内存到寄存器的数据传输，由于这是为后续MMA指令服务的，我们以warp瓦片/MMA瓦片和warp为单位思考。遵循Flash Attention 2（第3.3节），我们让线程块中的每个warp处理一部分`tile_Q`，沿Q序列长度维度分割。这意味着不同的warp将索引到`tile_Q`的不同块，但它们都索引到相同的`tile_K`和`tile_V`块在KV序列长度循环中。
...
只有Q在MMA中充当A。K和V都在其MMA中充当B，尽管K需要转置的`ldmatrix`以获取正确布局（所有张量在全局和共享内存中使用行主序布局）。
...
- `(lane_id % 16)`和`row`在`(lane_id / 16 * 8)`中为每个线程选择正确的行地址，遵循所需的乘数A布局（见上图）。`col`是

`ldmatrix_x4()`的一个小包装，为了方便。您可以参考`ldmatrix.sync.aligned.m8n8.x4.b16`common.h[获取更多细节。](https://github.com/gau-nernst/learn-cuda/blob/e83c256/07_attention/common.h)K和V可以类似地从共享内存加载到寄存器内存。需要注意的一点是关于使用

时的行主序/列主序布局。无论是否使用`ldmatrix`修饰符，每个线程仍提供8x8瓦片中每行的行地址。`.trans`只改变`.trans`结果的**寄存器布局**。`ldmatrix` results.
...
### 我们有了高层瓦片化设计，并知道如何为MMA加载数据。调用MMA很简单——只需在我们的代码中插入
...
Online normalizer calculation for softmax[和Flash Attention 2论文。](https://arxiv.org/abs/1805.02867)我们有以下softmax的数学定义。对于每行长度为

，是最大减法以提高数值稳定性（如果输入很大，$L\_{kv}$
...
$-m$很容易爆炸）。让我们提取分母归一化器并将整行写为向量。$\exp(\cdot)$在我们的第二个矩阵乘法
...
中，P（softmax输出）的每行与V的相应列进行点积。`O += P @ V`额外的点积是因祸得福——我们不再需要行中的单个元素来获取最终结果。这使得Flash Attention能够单次计算注意力。为了更清楚地看到这一点，让我们考虑在线计算期间添加新元素的迭代过程。

$$
o=\vec P \cdot \vec V = \frac{1}{\sum\_{l\in\[0,L\_{kv})}\exp(s\_l-m)} \sum\_{l\in\[0,L\_{kv})}\exp(s\_l-m) \cdot v\_l
$$

我在这里滥用符号，但我希望传达这个想法。当我们添加新元素
...
\displaystyle\sum\_{l\in\[0,L)}\exp(s_l-m\_{\[0,L)})$}\colorbox{orange}{$\displaystyle\exp(s_L-m\_{\[0,L+1)})$} + \colorbox{lime}{$方程意味着我们只需要在添加$}$

之前$\colorbox{red}{rescale}$重新缩放$\colorbox{orange}{previous normalizer}$。同样的逻辑可以应用于与V的点积（未归一化输出）。$\colorbox{lime}{new term}$这是在线softmax和Flash Attention的关键思&#x60F3;**。**&#x5B9A;义
...
是迄今为止看到的元素的最大值，$m$是$\tilde{o}$未归一化**输出，**&#x662F;归一化器。我们需要$\mathrm{sumexp}$来计算如上所示的重新缩放因子。$m$您可以确信更新注意力状态是一个

结合性**操作——元素用于更新注意力状态的顺序无关紧要。**&#x8FD9;种结合性属性使得像
...
Flash Decoding[这样的东西成为可能，这是注意力的split-K版本。](https://pytorch.org/blog/flash-decoding/), a split-K version of attention.

### 我们现在可以在我们的高层Python实现中填补在线softmax的空白。
...
#### 当将其转换为CUDA C++时，最棘手的部分是理解MMA布局。让我们从

开始。`tile_S`.

![MMA m16n8k16 output layout](/images/others/others-fa-5090/006-c90e5d1a.png)

NVIDIA PTX文档[。](https://docs.nvidia.com/cuda/parallel-thread-execution/#warp-level-matrix-fragment-mma-16816-float)Softmax缩放对所有元素应用相同的缩放，所以这很简单。接下来，我们需要计算当前瓦片的行最大值。记住我们这样分配
...
`4`在上图中，即每个线程持有来自2行的2个连续元素。要在行内（MMA输出瓦片）进行归约，我们对线程持有的2个连续元素进行归约，然后在4个线程组内进行归约，即`c0,c1,c2,c3`、`T0-T3`等。然而，行归约实际上在整个`T4-T7`内，因此我们还需要循环`tile_S`的`BLOCK_KV / MMA_N`。这可以与线程级归约结合，在4线程归约之前进行。`S_rmem`. This can be combined with thread-level reduction before 4-thread reduction.

![Row reduction](/images/others/others-fa-5090/007-9e2fcb2f.svg)

在典型的归约内核中，当只剩下32个活动线程时，我们可以使用warp shuffle
...
\_\_shfl_down_sync()[将数据从较高通道复制到较低通道，最终结果存储在线程0中。在这种情况下，由于我们需要最大值在组内的4个线程之间共享（用于后续的最大减法），我们可以使用](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#warp-shuffle-functions)以避免额外的广播步骤。`__shfl_xor_sync()` to avoid an additional broadcast step.

![Butterfly reduction](/images/others/others-fa-5090/008-f1f2f566.svg)

Butterfly reduction within 4 threads using \_\_shfl_xor_sync().
...
We don’t rescale `rowsumexp`这里是因为我们想稍后将其与新 sumexp 项的加法融合，即 FMA - 融合乘加。我们无法将乘法与 MMA 融合，因此需要为`O_rmem`进行单独的乘法。

#### 打包到（并计算行和 exp）

对于下一部分，我们将再次循环行维度（`BLOCK_KV / MMA_N`），以计算和打包`tile_P`从`tile_S`，同时进行 sumexp 的归约。回想一下，我们为`S`和`P`声明寄存器如下。
...
这意味着对于所有线程，`S_rmem`中的每 2 个浮点数可以打包为 BF16x2 在`P_rmem`的单个 32 位寄存器中，这正是`mma.m16n8k16`对第 2 个 MMA 的期望。没有跨线程的数据移动。注意，这并不总是成立：如果我们对第 1 个和/或第 2 个 MMA 使用 INT8 或 FP8 MMA，我们将需要跨线程置换数据以打包`tile_S`到`tile_P`。
...
无论如何，现在我们需要一个脚本来进行正确性检查以及速度基准测试。我更喜欢在 Python PyTorch 中做这些事，因为它容易操作，并且可以轻松地与其他具有 PyTorch 绑定的注意力内核进行比较。为此，我创建了：
...
对于正确性检查，我对比`F.sdpa()`，它应该默认调度 Flash Attention 2（至少在我的 GPU 和当前 PyTorch 版本上）。我还特意为随机输入添加了一个小偏置，这些输入是从标准正态分布中采样的，以便输出具有正均值。这是为了避免由零均值引起的大相对误差。
...
对于速度基准测试，通常一个好的做法是对比（1）硬件的理论极限，即光速，以及（2）已知的良好实现。我对注意力的计算限制区域更感兴趣，因此我将使用 FLOPS（每秒浮点运算次数，大写 S）作为比较指标。

要计算给定内核的 FLOPS，我们计算所需的浮点运算次数（FLOPs，小写 s），然后除以延迟。仅从 MMA 计算 FLOPs 应该足够好，结果是`4 * bsize * num_heads * len_q * len_kv * head_dim`。

“已知的良好实现”是`F.sdpa()`的 FA2 和 CuDNN 后端，以及[flash-attn](https://github.com/Dao-AILab/flash-attention)包的 FA2。对于我的内核，我确实对`BLOCK_Q`和`BLOCK_KV`进行了一些调优，并获得了以下结果。
...
对于第一个版本来说，看起来还不错，但我们仍有提升空间。这没关系，因为我们在下一个版本中还有一些技巧。实际上，这些技巧与优化矩阵乘法内核时使用的技巧完全相同。

#### 性能分析

在进入下一个版本之前，我想谈谈性能分析工具。我认为使用性能分析作为优化指南总是一个好主意。以前我只知道如何使用[ncu](https://docs.nvidia.com/nsight-compute/NsightComputeCli/index.html)在非常基础的层面。看到这么多人使用[Nsight Compute](https://developer.nvidia.com/nsight-compute)并带有酷炫的图表，我决定学习如何使用它，实际上它相当容易使用。

Nsight Compute 可以在 macOS 上运行，通过 SSH 访问另一台具有 NVIDIA GPU 的机器，这正是我当前使用的设置（是的，我专门在我的 Macbook 上写代码）。如果您不熟悉 Nsight Compute，我建议观看一两个教程来熟悉它。
...
**Stall Math Pipe Throttle**最高是好的 - 这意味着 Warp 忙于数学运算，即 Tensor Cores。第二高的是**Stall Short Scoreboard**。这通常意味着等待对共享内存的访问。您可以查看[Nsight Compute 文档](https://docs.nvidia.com/nsight-compute/ProfilingGuide/index.html)并搜索`stalled_short_scoreboard`。

我们可以通过查看**Memory Workload Analysis**来双重确认这一点，它揭示了几个问题。

![v1 的内存分析](/images/others/others-fa-5090/011-2b867178.png)

内核 v1 的内存分析。

- **L1TEX Global Store Access Pattern**来自存储输出，因为它是我们唯一的全局写入。这不重要，因为当`len_kv`很大时，循环 KV 序列长度的运行时间应占主导。
- **L1TEX Local Load/Store Access Pattern**是由于寄存器溢出。由于是寄存器溢出，每次仅溢出和重新加载 1 个元素是正常的。减少`BLOCK_Q`（以便我们使用更少的寄存器来保存累加器）将解决此问题，但我的手动调优显示，一些溢出实际上更快。
- **Shared Load Bank Conflicts**正是我们正在寻找的 - 导致“Stall Short Scoreboard”的存储体冲突。

NVIDIA GPU 的共享内存由 32 个存储体支持。连续的 4 字节内存地址被分配给连续的存储体。当我们使用`ldmatrix`从共享内存加载数据到寄存器内存时，这带来了问题。尽管在任何文档中没有明确说明，`ldmatrix.x2`和`ldmatrix.x4`每次操作一个8x8的瓦片（tile）。这很好，因为它简化了我们的分析：我们只需要考虑加载一个8x8瓦片的情况。

考虑一个形状为8x64、数据类型为BF16的2D瓦片（tile），位于共享内存中。

![Bank conflicts](/images/others/others-fa-5090/012-a7a0793c.svg)

共享内存中8x64 BF16瓦片（tile）的内存存储体（bank）分布。

从上图可以看出，当我们加载8x8`ldmatrix`瓦片（tile）时，相同的4个存储体（bank）0-3服务所有32个线程，导致8路存储体冲突（bank conflict）。我不确定为什么Nsight Compute报告如上所示的16路存储体冲突（bank conflict）。我尝试查找[matmul blogposts with swizzling](https://alexarmbr.github.io/2024/08/10/How-To-Write-A-Fast-Matrix-Multiplication-From-Scratch-With-Tensor-Cores.html)和[NVIDIA forum threads](https://forums.developer.nvidia.com/t/ncu-detects-bank-conflicts-in-matrix-transposition-after-padding/239100/6)，并发现另一种检查存储体冲突（bank conflict）的方法是转到Nsight Compute的**Source**选项卡并检查**L1 Wavefronts Shared**和**L1 Wavefronts Shared Ideal**（我必须手动启用这两列，因为默认情况下它们没有显示）。

![Bank conflicts in ldmatrix](/images/others/others-fa-5090/013-f80df431.png)

内核v1中`ldmatrix`的实际和理想L1波前共享（L1 Wavefronts Shared）。

比率**Actual / Ideal**为8，符合我们关于8路存储体冲突（bank conflict）的假设。我仍然不确定为什么这个值与**Details**选项卡中的值存在差异。

无论如何，这个问题有两种标准解决方案

1. **填充共享内存（Pad shared memory）**。由于`ldmatrix`的对齐要求，我们只能将宽度填充16字节，相当于4个存储体（bank）。这意味着当我们转到下一行时，内存存储体（bank）会偏移4，从而避免存储体冲突（bank conflict）。在许多情况下，这已经足够好了。然而，这通常相当浪费，因为我们没有利用填充的存储空间。
1. **交换共享内存地址（Swizzle shared memory address）**。这是黑魔法：你将共享内存地址与一些魔术数字进行XOR运算，然后突然存储体冲突（bank conflict）就消失了！

让我们详细说明第二种方法。我不够聪明发明这个技巧，但至少我希望我能给出一些关于为什么它合理的提示。我们使用XOR是因为这个操作能很好地置换数据——在给定固定第二个输入的情况下，输入和输出之间存在一一映射。我们遇到存储体冲突（bank conflict）是因为当我们转到下一行时，我们再次命中相同的内存存储体（bank）->我们可以使用这个行索引来置换地址。

具体来说，如果我们查看原始行地址：

- **Bits 0-3**由于16字节对齐约束始终为零。
- **Bits 2-6**决定存储体索引（bank index）。我们只需要关心位4-6，因为低位始终为零（由于对齐）。
- 行步长（row stride）决定了当我们转到下一行时哪些位会递增（根据定义）。如果我们的2D瓦片（tile）宽度为64个BF16元素，行步长（row stride）为128字节。转到下一行将递增位7，留下**bits 0-6 unchanged**（但我们不关心位0-3）。
- 因此，我们可以将行地址的**bits 4-6**与行索引的**bits 0-2**进行XOR运算，这保证每行都会改变。

如果瓦片（tile）宽度不同，例如32个BF16，我们可以进行相同的推理。还要注意行索引编码在行地址内，因此我们只需要行地址和行步长（row stride）来进行交换（swizzling）。
...
要启用这种交换（swizzling），我们需要将其添加到`cp.async`（写入共享内存）和`ldmatrix`（从共享内存读取）调用中。
...
由于这是矩阵乘法（matmul）内核中的标准优化，我还为`ldmatrix`添加了一个小优化。我在主循环外预计算行地址和交换（swizzling），这样在热循环中工作更少。当我们在一个warp瓦片（tile）内迭代MMA瓦片（tile）时，我们需要递增地址。然而，交换（swizzling）是一个XOR操作，我们不能简单地将XOR与加法交换，即`(a + b) ^ c != (a ^ c) + b`。注意，如果基地址`a`有一些对齐，加法就变成了XOR！即`100 + 001 == 100 ^ 001`。因此，当递增`ldmatrix`的输入地址时，我们将其与列偏移进行XOR运算，而不是进行加法。行偏移会影响高于交换位（swizzled bits）的位，所以我们可以对其保持加法。
...
我们可以用Nsight Compute验证不再有存储体冲突（bank conflict）。基准测试结果显示有显著的提升（我总是为新版本的内核重新调整`BLOCK_Q`和`BLOCK_KV`）。
...
## 版本3 - 2级流水线（2-stage pipelining）

![Warp state statistics of v2](/images/others/others-fa-5090/014-e3f53ee7.png)

内核v2的Warp状态统计（Warp state statistics）。

**Stall Short Scoreboard**不再是问题，因为我们已经用交换（swizzling）处理了它。现在的问题是：
...
到目前为止，我们还没有重叠全局内存操作与计算操作（MMA）。这意味着在等待全局->共享传输完成时，Tensor Cores处于空闲状态。这似乎是引入**流水线（pipelining）**&#x6216;**双缓冲（double-buffering）**&#x7684;好时机：分配比需要更多的共享内存，这样我们可以在处理当前迭代时预取下一个迭代的数据。

- 技术上我们也可以流水线共享->寄存器数据传输。这实际上在CUTLASS的[Efficient GEMM doc](https://github.com/NVIDIA/cutlass/blob/v4.1.0/media/docs/cpp/efficient_gemm.md)中提到过。然而，我在我的5090上从未成功实现过。检查我当前代码生成的SASS，我看到`LDSM`（PTX中的`ldmatrix`）和`HMMA`（半精度`mma`在PTX中）之间存在交错，可能由编译器完成以实现类似的内存-计算重叠效果。

让我们讨论**N级流水线（N-stage pipelining）**&#x7684;更通用实现。这个[NVIDIA blogpost](https://developer.nvidia.com/blog/controlling-data-movement-to-boost-performance-on-ampere-architecture/)很好地解释了这个想法，但通常我不太喜欢使用CUDA C++ API（考虑到CUTLASS也不使用，我认为直接使用PTX更有趣）。N级意味着在任何时间点都有N个进行中的阶段。这将是我们在内循环中想要保持的**不变性（invariance）**。

- 这与`num_stages`中提到的概念相同，用于自动调优（autotuning）。[triton.Config](https://triton-lang.org/main/python-api/generated/triton.Config.html)双缓冲（double buffering）是N=2的特殊情况。
- NVIDIA工程师/架构师为我们提供了
...
和`cp.async.commit_group`来优雅地实现这一点。`cp.async.wait_group`：一个

- `cp.async.commit_group`组（group）`cp.async`自然地映射到流水线中的一个预取阶段。
- `cp.async.wait_group N`：意味着等待直到最多剩下N个进行中的组（group）。如果我们做`cp.async.wait_group num_stages-1`，这意味着我们等待直到最早的预取完成（记住，我们总是有`num_stages` 进行中的预取作为循环不变量）。

在我们实现注意力（attention）的案例中，有两个小改动。

1. 由于我们已经为K和V消耗了大量共享内存（shared memory），且[消费级GPU（consumer GPUs）通常具有适中的共享内存大小](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#features-and-technical-specifications-technical-specifications-per-compute-capability) 相比其服务器对应物，我决定保持为2级流水线（2-stage pipeline），这也使代码略微简化。
1. 我们可以拆分K和V的预取，因为发出V预取可以延迟到第1个MMA之后。第二个改动需要一些微调：每个K和V预取是一个单独的`cp.async` 组（group）（以便我们可以独立等待它们）。

我从[Mingfei Ma](https://github.com/mingfeima)，PyTorch CPU后端的维护者那里学到的一个简洁编码风格是使用[lambda表达式（lambda expression）](https://github.com/pytorch/pytorch/blob/v2.8.0/aten/src/ATen/native/cpu/int8mm_kernel.cpp#L63) 来编写预取代码。它实现了两个好处：（1）保持相关代码靠近调用站点，（2）使多次调用同一代码块非常清晰。
...
我实验了一下在循环中放置`load_K/V` 和 `cp.async.wait_group` 的位置，发现上述放置产生了最佳性能。虽然最终这取决于编译器如何重新排列和交错不同指令，但上述放置是合理的：将`load_V()` 放在第1个MMA之后，以便当K数据在寄存器中时张量核心（Tensor Cores）可以立即开始工作（而不是等待发出V的`cp.async`），即保持张量核心忙碌；`load_V()` 放在在线softmax之前以保持内存引擎（memory engine）忙碌（而CUDA核心正在处理在线softmax）。同样，最优放置也可能很大程度上取决于硬件，例如内存和计算相对速度、不同内存和计算单元是否可以同时工作……
...
对于最后两个版本，我无法从性能分析数据中识别出任何优化机会（可能只是技能问题）。想法主要来自阅读随机资料和盯着内核。

之前，我们使用`ldmatrix.x2` 用于K和V，因为它自然适合`n8k16` MMA瓦片（MMA tile）。然而，既然我们无论如何处理更大的瓦片，我们可以直接使用`ldmatrix.x4` 来发出更少的指令。有两个选项：加载`n16k16` 瓦片，或`n8k32` 瓦片。

![对B使用ldmatrix.x4](/images/others/others-fa-5090/016-78fcfc89.svg)

对被乘数B使用ldmatrix.x4的可能选项。

一个选项比另一个更好吗？我们可以尝试在算术强度方面做一些分析。乍一看，`n16k16` 看起来是更好的选项：2个`ldmatrix.x4`（1个用于A和1个用于B）来做2个`mma.m16n8k16`；而`n8k32` 选项需要3个`ldmatrix.x4`（2个用于A和1个用于B）来做2个`mma.m16n8k16`。如果我们要为矩阵乘法内核实现这个想法，这个分析会有意义。然而，在我们的案例中，被乘数A（查询）已经在寄存器中，因此我们只需要考虑被乘数B（键和值）的加载成本。这个认识表明两个选项应该相同。
...
我对加速感到相当惊讶。这个版本唯一的区别是我们在主循环中使用了2倍更少的`ldmatrix` 指令。然而，我们获得了显著的改进，接近SOL。我猜由于张量核心和内存引擎在新GPU中如此快速，调度和发出指令可能成为瓶颈！

## 版本5 - 更好的流水线

在版本3中，我们为K和V都使用双缓冲区。然而，这是冗余的：在执行第1个MMA时，我们可以预取当前迭代的V；在执行第2个MMA时，我们可以预取下一个迭代的K。换句话说，我们只需要为K使用双缓冲区。
...
更高效地使用共享内存意味着我们可以增加一些瓦片大小。我将`BLOCK_KV` 从32增加回64。增加`BLOCK_Q` 很困难，因为它会使保存累加器的寄存器数量翻倍。改进是适度但明显的。
...
回顾过去，我们的内核v3已经击败了官方的Flash Attention内核，这是一个不错的惊喜。感觉相比前几代，从5090获得良好性能相当容易。然而，我们最好的内核落后于CuDNN的意味着仍有提升空间。我尝试检查CuDNN注意力内核的性能分析数据，并得到以下细节
...
- 共享内存：40.96 Kb -> 那是`40960 / (64 * 128 * 2) = 2.5` 乘以`(BLOCK_KV, DIM)`。缓冲区的分数数量相当奇怪。或者他们的内核更像`BLOCK_KV=32` 和5个缓冲区？我不知道。

无论如何，这里有一些有趣的想法可以在此基础上构建（除了尝试击败CuDNN）：

1. 实现反向传播（backward pass）（我听说这比前向传播难得多）
1. 量化/低比特注意力（quantized/low-bit attention），特别是5090上的NVFP4。我相信[SageAttention](https://github.com/thu-ml/SageAttention) 是这个领域的开源前沿。
1. 使用TMA（即`cp.async.bulk`）与warp-specialization设计。[Pranjal](https://x.com/pranjalssh) 为此为H100矩阵乘法写了一篇[不错的博客文章](https://cudaforfun.substack.com/p/outperforming-cublas-on-h100-a-worklog)。
