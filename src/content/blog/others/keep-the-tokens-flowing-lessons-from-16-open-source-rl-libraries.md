...
| 每个 rollout 的输出长度 | 总输出令牌数（512 个 rollout） | 在 1×H100 上的时间（7B @ \~6K 令牌/秒） | 在 1×H100 上的时间（32B @ \~1.2K 令牌/秒） |
| :---------------------- | :----------------------------- | :-------------------------------------- | :----------------------------------------- |
| 2K 令牌（短 CoT）       | 约 1M 令牌                     | **约 3 分钟**                           | **约 14 分钟**                             |
| 8K 令牌（中 CoT）       | 约 4M 令牌                     | **约 11 分钟**                          | **约 56 分钟**                             |
| 32K 令牌（长 CoT）      | 约 16M 令牌                    | **约 45 分钟**                          | **约 3.7 小时**                            |
...
| 编排类型              | 定义                                                                                                                                    | 库                                                                                                 | 权衡                                                                                                |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| **分布式 Actor 模型** | 组件是*actors*，具有邮箱的隔离有状态进程，由运行时管理，处理调度、资源放置、容错和对象传输。通信通过异步 RPC / futures / 对象存储进行。 | **Ray：** verl、SkyRL、NeMo-RL、SLIME、MILES、ROLL、OAT、open-instruct。**Monarch：** TorchForge。 | 最丰富的抽象；开箱即用地解决调度和容错。增加了非平凡的运行时依赖和框架特定的调试开销。              |
| **原生 Python 并发**  | 组件是线程、协程（`asyncio`）、`threading` 原语、`multiprocessing` 子进程和队列。无外部编排运行时。                                     | verifiers-rl、PipelineRL（池内）、ART（`asyncio` + 子进程代理）、AReaL（`asyncio` 基于事件循环）   | 最小依赖，易于调试，完全控制。限于单节点，除非与额外 IPC（Redis、HTTP、NCCL）配对用于多节点通信。   |
| **Pub/Sub 消息总线**  | 组件是通过仅追加流或消息队列通信的解耦生产者和消费者。本身不是编排，而是*数据传输层（data transport layer）*，连接独立运行的池。        | PipelineRL（池间：Redis `XADD`/`XREAD` 流用于多节点，仅追加 JSONL 文件用于单节点）                 | 跨池边界清晰解耦，无需 RPC。不管理进程生命周期、调度或故障恢复；必须与另一种编排类型配对。          |
| **HTTP 微服务**       | 组件是通过 REST API 通信的独立服务。语言无关，最大解耦。                                                                                | Atropos                                                                                            | 任何推理服务器，任何语言，零共享状态。最高延迟（如果使用 NCCL）；无共享对象存储；容错是用户的责任。 |
...
| **双缓冲区**（一步提前） | 1    | verifiers-rl、SLIME（异步模式）、MILES、OAT                                                                                                    | 在训练步骤 N 开始时提交生成 N+1；恰好重叠一个批次 |
...
| **无界/流**              | 无限 | PipelineRL（Redis 流）、SLIME（完全异步模式）、Atropos                                                                                         | 连续生成；陈旧度仅受显式版本控制限制              |
...
| ----------------- | :------: | :------: | :-----: | ----------------------------------------------------------------------------------------------------- |
| **AReaL**         |    ❌    |    ✅    |   ⚠️    | `max_head_offpolicyness` 容量公式；可选 `use_decoupled_loss` 添加上限为 5.0 的 IS 权重                |
| **ART**           |    —     |    —     |    —    | 同步模式；训练前收集全部 rollout；设计上无陈旧性                                                      |
| **Atropos**       |    ❌    |    ✅    |   ❌    | `max_batches_offpolicy`，缓冲批次数上限                                                               |
| **MILES**         |    ❌    |    ❌    |   ✅    | TIS + OPSM                                                                                            |
| **NeMo-RL**       |    ✅    |    ❌    |   ❌    | `max_trajectory_age_steps`，逐样本版本丢弃                                                            |
| **OAT**           |    ❌    |    ❌    |   ✅    | 截断 TIS 比率                                                                                         |
| **open-instruct** |    ❌    |    ✅    |   ⚠️    | `async_steps` 上限（默认 1，生产 8）；可选 `--truncated_importance_sampling_ratio_cap ρ` 添加截断 TIS |
| **PipelineRL**    |    ✅    |    ❌    |   ❌    | `max_lag`，每样本整数版本标签；超过阈值则丢弃                                                         |
| **PRIME-RL**      |    ✅    |    ✅    |   ✅    | 完整混合策略：`max_async_level` 版本差距 + `max_off_policy_steps` 取消 + IPO 信任域 IS                |
| **ROLL**          |    ❌    |    ❌    |   ✅    | 最丰富的 IS 套件：TIS、TOPR、CISPO、Kimi15，六种离策略损失变体                                        |
| **SkyRL**         |    ❌    |    ✅    |   ❌    | `max_staleness_steps`，容量门控在超过时阻止新 rollout                                                 |
| **SLIME**         |    ❌    |    ❌    |   ✅    | TIS + OPSM（部分 rollout 的离策略掩蔽）                                                               |
| **TorchForge**    |    ✅    |    ❌    |   ❌    | `max_policy_age`，每样本版本标签；硬丢弃                                                              |
| **Tunix**         |    ❌    |    ✅    |   ❌    | 有界队列 + 每步同步；陈旧性在结构上受限                                                               |
| **verl**          |    ❌    |    ❌    |   ✅    | 截断 TIS 比率；可选 OPSM                                                                              |
| **verifiers-rl**  |    ❌    |    ✅    |   ❌    | 深度=1 FIFO + 每步同步；结构上不可能出现陈旧性                                                        |
...
| 策略                             | 库                                | 描述                                                                                                                                          |
| -------------------------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| **隐式延续**                     | PipelineRL                        | 序列从不被中断。权重在前向传播之间交换；序列以新权重继续。存储的 logprobs 仍然有效，因为训练使用*记录的* $\pi_{\text{old}}$，而非重新计算。   |
...
| ----------------- | ---------------------- | -------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------ |
| **AReaL**         | ✅ 是                  | 仅 FSDP2（不含 Megatron/Archon） | HF `peft`                                    | ✅ 是（基于磁盘同步；仅传输可训练参数；vLLM 适配器热交换）               |
| **ART**           | ✅ 是（主要设计）      | 两者均支持（共置 + 独立 GPU）    | Unsloth/`peft`（默认）；自定义 Megatron LoRA | ✅ 是（仅保存/加载适配器；进程内或 HTTP 适配器热交换；基础权重从不移动） |
| **Atropos**       | ✅ 是                  | 分离模式                         | HF `peft`                                    | ✅ 是（`lora_only` / `lora_restart` 模式）                               |
| **MILES**         | ✅ 是                  | 两者均支持（共置 + 分离）        | Megatron-Bridge                              | ✅ 是（SGLang 的适配器同步配置）                                         |
| **NeMo-RL**       | ✅ 部分支持\*          | 两者均支持                       | 自定义（非 `peft`）                          | ❌ 无证据                                                                |
| **OAT**           | ✅ 是                  | 两者均支持                       | HF `peft`                                    | ✅ 是（仅 LoRA 同步模式）                                                |
| **open-instruct** | ⚠️ 代码存在但未接入‡   | —                                | HF `peft`（仅 SFT/DPO）                      | ❌ 否（RL 训练器中未应用 LoRA）                                          |
| **PipelineRL**    | ✅ 是                  | 非共置                           | HF `peft`                                    | ❌ 否（完整 NCCL 广播）                                                  |
| **PRIME-RL**      | ✅ 是                  | 分离模式                         | 自定义 MultiLoRA（非 `peft`）                | ✅ 是（仅适配器状态字典提取）                                            |
| **ROLL**          | ✅ 部分支持†           | 仅 DeepSpeed 后端                | HF `peft` / TRL                              | ❌ 无证据                                                                |
| **SkyRL**         | ✅ 是                  | 两者均支持                       | `peft`（FSDP）/ Megatron-Bridge（Megatron）  | ✅ 是（基于文件系统的适配器同步）                                        |
| **SLIME**         | ❌ 否                  | —                                | —                                            | ❌ 否                                                                    |
| **TorchForge**    | ❌ 否                  | —                                | —                                            | ❌ 否                                                                    |
| **Tunix**         | ✅ 是                  | 两者均支持                       | qwix（JAX 原生）                             | ✅ 是（自动检测）                                                        |
| **verl**          | ✅ 是（最完整）        | 两者均支持                       | `peft`（FSDP）/ Megatron-Bridge（Megatron）  | ✅ 是（未合并适配器同步）                                                |
| **verifiers-rl**  | ✅ 是（通过 prime-rl） | 分离模式                         | HF `peft` + FSDP2 + vLLM                     | ✅ 是（vLLM LoRA 服务）                                                  |

\* NeMo-RL：GRPO 和 DPO 的 LoRA 仅在 DTensor 后端支持；Megatron Core 后端仅支持 SFT（RL LoRA 标注为"即将推出"）。使用自定义的 DTensor 兼容 LoRA 模块（非 `peft`），可选 Triton 融合内核。

† ROLL：LoRA 仅在 DeepSpeed 训练后端正式支持。Megatron 后端 LoRA 出现在 2026 年 2 月的更新日志中，但仍处于实验阶段。

‡ open-instruct：模型配置中暴露了 LoRA 相关字段（`use_peft`、`lora_r`、`lora_alpha`），且适配器保存已在检查点逻辑中处理。但在 RL 训练路径中从未初始化 `peft` 模型；截至 2026 年 3 月，LoRA 对 RL 训练器而言仍是仅限 SFT/DPO 的功能。
...
| **TorchForge**    | FSDP2                      | DP、TP、CP             | ✅ 通过 TorchTitan | ❌            |
| **Tunix**         | JAX/XLA                    | DP、TP                 | ❌ 自定义 Flax     | ❌            |
...
| ----------------- | -------------- | ----------------------------------------------------- | ---------------------- | ----------------------------------- | --------------------- | --------------------------- | ------------------------------------- | -------------------------------------------------------------------- | --------------------------------------- |
| **AReaL**         | inclusionAI    | 原生 Python（asyncio + HTTP RPC）；可插拔 Ray/Slurm   | vLLM、SGLang           | NCCL 分块传输或文件系统 safetensors | 深度限制 + IS（可选） | 🟧 软暂停（进行中序列完成） | FSDP2 或 Megatron-LM 或 Archon        | FSDP2：DP+SP+TP；Megatron：TP+SP+PP+CP+EP；Archon：FSDP2+TP+SP+PP+EP | ✅ `peft`（仅适配器）                   |
| **ART**           | OpenPipe       | 原生 Python（asyncio + mp 子进程）                    | vLLM                   | LoRA 适配器热交换（无完整权重传输） | 同步（无）            | ❌ 否                       | Unsloth（单 GPU）；Megatron-LM        | 无（Unsloth）；TP×EP×DP（Megatron）                                  | ✅ `peft` / Megatron LoRA（仅适配器）   |
| **Atropos**       | NousResearch   | HTTP 微服务（FastAPI）                                | vLLM、SGLang、OpenAI   | 文件系统检查点 + vLLM 重启          | 深度限制              | ❌ 否                       | 单 GPU PyTorch；TRL/Accelerate        | 无（原生）；通过 TRL 适配器支持 FSDP/ZeRO                            | ✅ `peft`（仅适配器）                   |
| **MILES**         | radixark       | 分布式 Actor（Ray）                                   | SGLang                 | NCCL 或 CUDA IPC                    | IS 校正               | 🟧 中止 + 回收到缓冲区      | Megatron-LM（主要）；FSDP2            | Megatron：TP×PP×DP；FSDP2 可用；共置 CUDA IPC                        | ✅ Megatron-Bridge（仅适配器）          |
| **NeMo-RL**       | NVIDIA         | 分布式 Actor（Ray）                                   | vLLM、SGLang、Megatron | NCCL 或 CUDA IPC-ZMQ 或 HTTP        | 版本拒绝              | ✅ 进行中序列延续           | DTensor（FSDP2+TP）或 Megatron-Bridge | DTensor：TP+SP+CP+FSDP2；Megatron：TP×PP×CP×EP×ETP + FSDP2           | 🟧 自定义（无仅适配器同步）             |
| **OAT**           | SAIL-SG        | 分布式 Actor（Ray）                                   | vLLM                   | 逐参数 NCCL + ZeRO-3 聚合           | IS 校正               | ❌ 否                       | DeepSpeed ZeRO-2/3                    | ZeRO-2/ZeRO-3 DP；AutoTP                                             | ✅ `peft`（仅适配器）                   |
| **open-instruct** | AI2 (AllenAI)  | 分布式 Actor（Ray）                                   | vLLM                   | NCCL 广播；可选飞行中更新           | 深度限制 + IS（可选） | 🟧 排空或飞行中（可配置）   | DeepSpeed ZeRO-0/2/3                  | ZeRO-3 DP + Ulysses SP；vLLM TP（仅推理）                            | ❌ 否                                   |
| **PipelineRL**    | ServiceNow     | 原生 Python + Pub/Sub（asyncio + Redis/JSONL）        | vLLM                   | NCCL pg + HTTP 通知                 | 版本拒绝              | ✅ 隐式延续                 | DeepSpeed ZeRO-3                      | ZeRO-3 DP + Ring SP；ZeRO++ 可用                                     | ✅ `peft`（完整同步）                   |
| **PRIME-RL**      | PrimeIntellect | 原生 Python（asyncio + FS/ZMQ）                       | vLLM                   | 文件系统 safetensors + HTTP 或 NCCL | 版本 + 深度 + IS      | 🟧 组取消                   | 仅 FSDP2                              | FSDP2 逐块 + TP + CP + EP；pp=1                                      | ✅ 自定义 MultiLoRA（仅适配器）         |
| **ROLL**          | Alibaba        | 分布式 Actor（Ray）                                   | vLLM、SGLang           | 通过专用更新组的 NCCL               | IS 校正               | ❌ 否                       | DeepSpeed ZeRO 或 Megatron 或 FSDP2   | DS：ZeRO+Ulysses SP；Megatron：TP×PP×CP×EP；FSDP2：HSDP+Ulysses      | 🟧 `peft`（仅 DeepSpeed）               |
| **SkyRL**         | NovaSky-AI     | 分布式 Actor（Ray）+ 原生 Python                      | vLLM、SGLang           | NCCL pg                             | 深度限制              | 🟧 中止 + 带前缀重试        | FSDP/FSDP2 或 Megatron-Bridge         | FSDP：ZeRO 分片 + Ulysses SP；Megatron：通过桥接的完整 5D；JAX 后端  | ✅ `peft` / Megatron-Bridge（仅适配器） |
| **SLIME**         | THUDM          | 分布式 Actor（Ray）                                   | SGLang                 | NCCL pg，分桶                       | IS 校正               | 🟧 中止 + 回收到缓冲区      | Megatron-LM                           | TP×PP×DP；Megatron→HF 转换；MoE EP all-gather                        | ❌ 否                                   |
| **TorchForge**    | Meta           | 分布式 Actor（Monarch）                               | vLLM                   | torchstore + 共享内存预取           | 版本拒绝              | ❌ 否                       | FSDP2（通过 TorchTitan）              | FSDP2 + TP；CP 部分支持；PP 尚未实现                                 | ❌ 否                                   |
| **Tunix**         | Google         | 原生 Python（ThreadPoolExecutor + asyncio）；JAX 原生 | vLLM、SGLang、JAX      | 跨网格重分片                        | 深度限制              | ❌ 否                       | JAX/XLA 二维网格                      | 二维 JAX 网格：FSDP + TP；无 PP；以 TPU 为主                         | ✅ qwix / QLoRA（仅适配器）             |
| **verl**          | ByteDance      | 分布式 Actor（Ray）                                   | vLLM、SGLang           | NCCL + checkpoint-engine 分桶       | IS 校正               | ✅ 显式保存/恢复            | FSDP1/FSDP2 或 Megatron-Core          | FSDP：ZeRO-2/3/HSDP + Ulysses SP；Megatron：TP×PP×VPP×CP×EP×ETP      | ✅ `peft` / Megatron-Bridge（仅适配器） |
| **verifiers-rl**  | PrimeIntellect | 原生 Python（threading + asyncio）                    | vLLM                   | PyNCCL 广播                         | 深度限制（深度=1）    | ❌ 否                       | DeepSpeed ZeRO-3（Accelerate）        | 仅 ZeRO-3 DP；无 TP/PP                                               | ✅ `peft`（仅适配器）                   |
...
训练-推理不匹配问题在异步强化学习中普遍存在；每当在策略 $\pi_{\text{old}}$ 下生成展开数据并在 $\pi_\theta$ 下计算梯度更新时，这两个策略就会产生分歧。大多数库通过重要性采样校正或硬版本拒绝来解决此问题。但 DeepSeek-V3.2 的生产经验揭示了两个**结构性**的不匹配来源，这是重要性采样校正无法修复的。
...
**来源 2：采样截断掩码不匹配。** Top-p 和 top-k 采样在生成时截断词汇表，将低概率令牌排除在采样分布之外。在训练期间，完整词汇表对 $\pi_\theta$ 可见。这违反了重要性采样恒等式：$\pi_{\text{old}}$（截断）和 $\pi_\theta$（完整）的动作空间不同，因此对于采样期间被掩码的令牌，重要性采样比 $\pi_\theta(o_t)/\pi_{\text{old}}(o_t)$ 是未定义的。

DeepSeek-V3.2 的**保持采样掩码**解决方案：在采样期间记录截断掩码，并在训练前向传播中将其应用于 $\pi_\theta$，使两个策略在相同的词汇表子集上操作。这需要将掩码从推理服务器传递回训练器，这同样是当前库基础设施不支持的功能。
