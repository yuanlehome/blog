...
cover: '/images/others/keep-the-tokens-flowing-lessons-from-16-open-source-rl-libraries/014-d966e2aa.png'
lang: zh
translatedFrom: en
---
> **TL;DR**——对于那些没时间阅读5000字关于异步强化学习（RL）基础设施文章的人（我们理解，您有模型要训练）：
>
> - **问题：**在同步强化学习（RL）训练中，数据生成（模型推理以创建数据样本）主导了实际时间——在320 亿（32B）参数模型上，单个包含32K 令牌的批次可能需要*数小时*，而用于训练的GPU在此期间保持空闲。
> - **大家共同采用的解决方案：**将推理和训练解耦（分离）到不同的GPU池，通过rollout缓冲区（模型输出的临时存储）连接它们，并异步（无需等待）传输权重，这样双方都不必等待对方。
> - **我们调查了 16 个开源库**，这些库实现了这种模式，并在7 个维度上进行了比较：编排原语、缓冲区设计、权重同步协议、陈旧性管理、部分rollout处理、LoRA支持以及分布式训练后端。
...
| :------------- | :------------------ | :------------------------- | :---------------------------- |
| 2K令牌（短CoT）     | 约 1M令牌               | **约3分钟**                   | **约14分钟**                     |
| 8K令牌（中CoT）     | 约 4M令牌               | **约11分钟**                  | **约56分钟**                     |
| 32K 令牌（长CoT）    | 约 16M令牌              | **约45分钟**                  | **约3.7小时**                    |

即使在短端（2K令牌生成，使用7B模型），仅生成就消耗每个训练步骤数分钟。在长端，前沿推理模型越来越多地在此操作，单个生成阶段可能需要*数小时*在单个GPU上。扩展到8个推理GPU可将这些时间大致除以8倍（假设吞吐量线性扩展），但即便如此，32B模型上的32K 令牌展开仍需要约28分钟每步。

“**落后者问题（straggler problem）**&#x8FDB;一步加剧了这一点。在基于组的算法如GRPO中，每个提示采样G个完成。批次无法继续，直到*最慢*的完成结束。思维链输出长度变化很大；单个提示可能产生从1K到32K 令牌不等的完成。批次受最长完成限制，连续批处理仅部分缓解此问题：较短序列释放槽位用于新工作，但*最后一个*GRPO组中的序列仍会阻塞该组的奖励计算和训练步骤。
...
上表揭示了一个显著的模式：**调查的十六个库中有 8 个使用Ray作为其编排骨干**。这并非巧合；它反映了actor模型与RL训练结构之间的深层架构契合。[Anyscale（Ray背后的公司）对开源LLM RL库的调查](https://www.anyscale.com/blog/open-source-rl-libraries-for-llms)证实了这种趋同。大规模RL训练涉及本质上异构的组件（推理引擎、训练引擎、环境、奖励模型），这些组件必须在集群中编排，通常在不同硬件类型上，具有不同的扩展需求和故障模式。Ray的actor模型直接映射到这一点：
...
缓冲区控制有多少数据在飞行中。但数据只是方程的一半。另一半是在这些 rollout 变得陈旧之前将更新后的权重推回推理服务器——这就是权重同步的用武之地！

### 维度 3：权重同步协议


> **本维度专注于解聚模式**：推理和训练在独立的 GPU 池上运行，因为这正是异步重叠（以及权重同步设计）真正重要的部署拓扑。共置模式（同一组 GPU 承担两种角色）本质上是同步的，不存在下文讨论的传输/中断权衡问题。

这是架构影响最深远的维度。协议决定了同步延迟、中断粒度，以及部分 rollout 是否可行。

此处有一个关键区分：**传输机制**与**中断模型**。大多数库在粗粒度边界暂停生成（一次 HTTP 请求、一整批次甚至一整个训练步骤），然后才发起权重传输。PipelineRL 是个例外：它完全不会停止生成。
...
这正是 PipelineRL 与所有其他库从根本上分道扬镳之处。与其逐库列举，整个领域可归纳为几个概念层级，按中断粒度从细到粗排列：

| 中断粒度                                      | 行为描述                                                                                                                                             | 库                                                           |
| :--------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------- |
| **从不**（每次前向传播飞行中）                         | 序列永不中断。权重交换发生在 token 解码步骤之间（约 1–10 ms 间隙），正在运行的序列无缝继续使用新权重。                                                                               | PipelineRL, open-instruct (opt-in)                           |
| **每次 HTTP 请求**（中止 + 重同步）                  | 进行中的 HTTP 请求被中止，部分 token 通过前缀恢复机制重新提交或回收重试。                                                                                                 | SkyRL, SLIME, MILES                                          |
| **软暂停**（排空进行中任务）                          | 不接受新的生成请求，等待进行中的请求自然完成。排空后，权重完成同步，生成恢复。                                                                                                     | PRIME-RL, AReaL, open-instruct (default), verl (async)       |
| **每训练步骤/批次**（阻塞）                          | 生成必须完全结束，训练器与推理引擎轮流互相阻塞。                                                                                                                      | NeMo-RL, ROLL, OAT, TorchForge, Tunix, verifiers-rl, Atropos |

"从不停止"层级在质量上与所有其他层级截然不同：以 PipelineRL 为例，它在推理引擎中挂钩，使得锁的获取和释放*每次 transformer 前向传播*（一个序列的一个 token 步骤）发生一次。权重更新最多等待一次前向传播（约几毫秒），交换所有参数后生成立即恢复。所有其他库在更粗的边界停止生成，从一次 HTTP 请求（约数百毫秒）到整个批次边界（约数秒）不等。

权重同步控制新权重*何时*到达。但异步训练意味着 rollout 始终在*某个*策略版本下生成，而*生成*策略可能落后训练器数个梯度步骤。库如何处理这种策略滞后就是陈旧性管理。

### 维度 4：陈旧性管理

*系统如何处理生成的 rollout 可能来自比当前训练版本更旧的策略这一事实？*

一旦生成与训练重叠，样本便成为离策略的。管理此类陈旧性出现了三种**正交**策略，大多数生产系统会组合使用其中多种：

**策略 1：逐样本版本拒绝。** 每个样本都标记着生成它的整数策略版本。在训练时，版本落后当前策略超过阈值的样本在进入损失计算之前被硬丢弃。简单且正确，但浪费了生成被丢弃样本所耗费的宝贵算力。

**策略 2：深度限界。** 生成和训练之间的队列或缓冲区容量有界（或设有显式的陈旧性门），从架构上限制了任何样本可以落后多少。范围从 depth=1（一步超前双缓冲，从构造上保证不会产生陈旧性）到与版本差绑定的显式容量公式。无需逐样本版本跟踪；界限由系统的流水线深度强制执行。

**策略 3：IS 加权损失校正。** 到达训练器的陈旧样本通过重要性采样比 $\pi_{\text{old}}(a|s) / \pi_\theta(a|s)$ 进行重加权，通常会做截断（Truncated IS）。部分库还应用 OPSM（对具有负优势的离策略样本将损失清零）。此策略保留了吞吐量，不丢弃任何样本，但 IS 比率会带来梯度方差代价。

这些策略是正交的：系统可以单独使用版本拒绝、单独使用深度限界、单独使用 IS 校正，或其任意组合。同步系统通过从不重叠生成与训练来完全避免此问题。

| 库                 | 版本拒绝 | 深度限界 | IS 校正 | 关键配置/说明                                                                                                              |
| ----------------- | :---------------: | :------------: | :-----------: | -------------------------------------------------------------------------------------------------------------------- |
...
- **版本拒绝**简单正确，但当大量样本被丢弃时会浪费算力。
- **IS 校正**以梯度方差为代价保留吞吐量。
- **深度限界**是最粗粒度的机制，但完全避免了逐样本的簿记开销。

生产系统（PRIME-RL、AReaL、open-instruct）的趋势是采用**混合方法**，将深度限界与可选的 IS 校正相结合，在获得有界队列架构简洁性的同时，以重要性加权作为损失层面的安全网来保障训练稳定性。

陈旧性管理处理的是在旧策略下生成的数据。但当权重更新到来时，*仍在生成中*的数据又该如何处理？

### 维度 5：部分 Rollout 处理

*当权重更新到来时，正在进行中的生成会发生什么？*

这对于单次 rollout 可能需要数分钟的长上下文任务至关重要。以下是几种处理策略：

| 策略                                           | 库                                 | 描述                                                                                                                                                                                                                                                                         |
| --------------------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **隐式续生**                                      | PipelineRL                        | 序列永不中断。权重在前向传播之间交换，序列直接以新权重继续生成。存储的 logprob 保持有效，因为训练使用*记录的* π_old 而非重新计算。                                                                                                                                                                                          |
| **中止并通过前缀重试**                                | SkyRL, SLIME                      | 活跃序列被中止，部分 token 被积累后，使用新权重通过前缀恢复机制重新提交。                                                                                                                                                                                                                                |
| **显式保存/恢复**                                   | verl（完全异步）                         | rollout worker 将部分 token ID 和 logprob 保存到缓冲区，等待同步完成后从保存的前缀处恢复生成。                                                                                                                                                                                                        |
| **分组取消（生成继续）**                               | PRIME-RL                          | 陈旧的 rollout 分组的异步任务被取消；推理服务器继续处理进行中的 HTTP 请求，但结果会被丢弃。权重同步在 HTTP 请求之间触发，不中断请求内的生成过程。                                                                                                                                                                                  |
| **无部分 rollout 支持**                            | verifiers-rl, OAT, Atropos, Tunix | 权重同步仅在批次边界发生，进行中的生成必须完成后才能开始同步。                                                                                                                                                                                                                                        |
| **软暂停，进行中序列完成**                              | **AReaL**                         | 暂停信号阻止新的 KV 缓存分配，但不中止进行中的序列。任务调度器停止提交新任务，运行中的任务运行至完成。权重同步后，生成调度恢复。                                                                                                                                                                                                |
| **完全休眠，同步时无进行中请求**                           | **ART**                           | 按设计，训练仅在所有 rollout 收集完成后才开始，因此触发休眠时不存在进行中的序列。1 级休眠（存在进行中请求时）将 KV 缓存卸载到 CPU；2 级休眠则完全丢弃。                                                                                                                                                                           |
| **排空或飞行中（可配置）**                              | **open-instruct**                 | 默认：停止标志阻止新的预取，权重更新等待活跃任务排空。启用飞行中更新时，跳过排空步骤，在 token 仍在生成中广播权重；进行中的序列继续使用新旧权重混合。                                                                                                                                                                                  |

迄今为止，每个维度都假设进行全参数训练。但在 LoRA 训练中，训练的适配器参数只有几百万而非数十亿，权重同步问题几乎消失。下面来看看这些库如何支持 LoRA 训练。

### 维度 6：LoRA 训练支持

*该库是否支持通过 LoRA 适配器进行参数高效训练、支持哪些模式，以及是否利用了仅适配器权重同步？*

LoRA 可以说是 GPU 预算有限的团队最具实际意义的维度。它将可训练参数数量减少 99%+，峰值激活内存减半，并且当推理服务器支持 LoRA 时，可实现*仅适配器权重同步*：不再广播 7B+ 模型的每个参数（约 100–500 ms NCCL），而只将适配器增量推送到 vLLM，在 rank 32 下约为 50 MB，传输时间低于毫秒。
...
1. **HuggingFace `peft`**（PipelineRL、SkyRL/FSDP、verifiers-rl、ROLL、OAT、Atropos）：最常见的选择。标准检查点格式（`adapter_model.safetensors`），与任何 HF Transformers 训练循环兼容。ZeRO-3 交互需要注意：例如，OAT 需要禁用融合 LM head；ROLL 则必须完全禁用梯度检查点。

1. **Megatron-Bridge**（verl/Megatron、SkyRL/Megatron、MILES）：3D 并行训练（TP × PP × DP）所必需。支持多种 LoRA 类型：`lora`、`canonical_lora`（将合并的QKV拆分为独立的Q/K/V适配器），`vlm_lora`，以及`dora`。该`canonical_lora`变体避免了QKV合并，从而提高了训练稳定性。MILES以HF`peft`格式和Megatron原生每秩格式保存检查点。
...
以上涵盖了七个维度，每个维度捕捉了同一底层问题的不同侧面。合在一起，它们为比较各库提供了完整的视角。是时候将它们汇总到一张表上了。

***

## 4. 全局概览：十六个库一览

> **注：** 本概览反映截至 2026 年 3 月各库的状态。生态系统正在快速演进，具体功能、后端和集成可能在近期发生变化。
...
这是当前的发展状态。但该领域正在快速演进，几个新兴趋势即将以设计者可能未曾预料的方式对这些架构进行压力测试。

***

## 5. 下一波浪潮：设计启示

以下趋势并非新技术目录；每种趋势都会对当前的基础设施和算法选择产生具体压力。问题不是"前沿是什么？"而是"如果这一趋势胜出，我的当前技术栈中哪些地方会出问题？"

### 5.1 无批评者算法：内存释放，但权重同步压力增加

PPO 的价值网络使任意训练节点的内存占用翻倍。该领域正在收敛于无批评者变体（GRPO、REINFORCE++、Online DPO），正是因为长思维链推理使这一开销在 8K–64K 上下文长度下难以承受。

**这解锁了什么：**&#x6D88;除评论家（critic）可释放约 50% 的训练GPU内存。这些空闲资源可重新分配给：（a）更大的rollout批次，直接减少掉队者（straggler）方差问题，或（b）在同一GPU上共同部署推理和训练，从而完全消除对单独的NCCL权重同步进程组的需求。
...
**关键含义：**&#x57FA;于PRM的信用分配打破了奖励计算廉价的假设。对来自7B模型的32K 令牌推理轨迹进行PRM前向传递可能非常昂贵。在G=8个完成（completion）每提示下，奖励计算可能消耗相对于生成本身不可忽略的挂钟时间。两个后果：
