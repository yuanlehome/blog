...
考虑一组/块权重 $w$，线性运算为 $y=wx$，量化后的对应为 $y=Q(w)x$。量化函数定义为：
...
其中 $N$ 为量化比特数，$\Delta$ 为由绝对最大值确定的量化尺度（scale）。考虑某个权重元素 $w\in w$，若将其乘以 $s>1$ 并对输入 $x$ 进行反向缩放，则得到 $Q(w\cdot s)(x/s)$：
...
其中 $\Delta'$ 为施加 $s$ 后的新量化尺度。经验上：（1）$\mathrm{Round}(\cdot)$ 的期望误差（记为 $\mathrm{RoundErr}(\cdot)$）近似不变，可视作在 $[0,0.5]$ 上均匀分布，平均误差约 0.25；（2）放大单个元素通常不会改变该组的最大值，因此 $\Delta'\approx \Delta$；（3）$\Delta$ 与 $x$ 以 FP16 表示，可忽略其量化误差。于是式(1)(2)的量化误差可写为：
...
当 $\Delta'\approx \Delta$ 且 $s>1$ 时，误差相对比例约为 $\frac{\Delta'}{\Delta}\cdot\frac{1}{s}\approx \frac{1}{s}$，因此显著权重的相对误差更小。
...
为同时考虑显著与非显著权重，我们对每个（输入）通道搜索一个缩放因子 $s$，使得量化后输出与原输出的差异最小：
...
其中 $Q(\cdot)$ 为权重量化函数（如 INT3/INT4，group size 128）， $W$ 为 FP16 权重， $X$ 为从小规模校准集缓存的输入特征（我们从预训练数据中采样以避免对某一任务过拟合）。 $s$ 为逐通道缩放； $\mathrm{diag}(s)^{-1}\cdot X$ 通常可融合到前一算子中。由于 $Q(\cdot)$ 不可微，无法用标准反向传播直接优化；基于近似梯度的方法也可能出现不稳定收敛。我们进一步分析影响缩放选择的因素，从而定义更稳定的搜索空间。由于通道显著性由激活尺度决定（“activation-awareness”），我们使用如下形式：
...
其中 $s_X$ 为激活的逐通道平均幅值， $\alpha$ 为单个超参数，用于平衡显著与非显著通道保护强度。我们在区间 $[0,1]$ 上做快速网格搜索（网格大小 20）选择最优 $\alpha$：0 表示不缩放，1 表示搜索空间中最激进的缩放。我们还结合权重裁剪（clipping）以最小化量化 MSE。表 3 给出了在 OPT、INT3‑g128 下的消融：AWQ 持续优于 RTN，并在硬件友好性更强的同时，达到与“1% FP16 混合精度”相近的精度。
