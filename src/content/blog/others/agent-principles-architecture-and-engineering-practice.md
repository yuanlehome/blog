...
source_author: Tw93
imported_at: '2026-03-25T11:02:29.281Z'
source:
  title: tw93.fun
  url: 'https://tw93.fun/2026-03-21/agent.html'
cover: /images/others/agent-principles-architecture-and-engineering-practice/001-38b59369.webp
lang: zh
---

![Agent 架构封面图](/images/others/agent-principles-architecture-and-engineering-practice/001-38b59369.webp)
...
![Agent Loop 控制流](/images/others/agent-principles-architecture-and-engineering-practice/002-50f693ec.svg)
...
![Workflow 与 Agent 对比](/images/others/agent-principles-architecture-and-engineering-practice/003-9ecd8040.svg)
...
1. **路由 Routing**：对输入分类，定向到对应的专用处理流程，简单问题走轻量模型，复杂问题走强模型，技术咨询和账单查询走不同逻辑。

1. **并行 Parallelization**：两种变体：分段法把任务拆成独立子任务并发跑，投票法把同一任务跑多次取共识，适合高风险决策或需要多视角的场景。

1. **编排器-工作者 Orchestrator-Workers**：中央 LLM 动态分解任务，委派给工作者 LLM，综合结果，nanobot 的 `spawn` 工具和 learn-claude-code 的子 Agent 模式都是这个原型。

1. **评估器-优化器 Evaluator-Optimizer**：生成器产出，评估器给反馈，循环直到达标，适合翻译、创意写作这类质量标准难以用代码精确定义的任务。

![五种常见控制模式](/images/others/agent-principles-architecture-and-engineering-practice/004-981be3e8.svg)
...
1. **约束编码化而非文档化**：写在文档里的规范很容易被忽略，编码进 Linter、类型系统或 CI 规则里的约束才具备可执行性，架构分层靠自定义 Linter 机械强制，不靠人工 Review。
1. **Agent 端到端自主完成任务**：从验证当前状态、复现 Bug、实现修复、驱动应用验证，到开 PR、处理 Review 反馈、自主合并，全链路不需要人介入，查日志、查指标、查追踪都由 Agent 主动完成。
1. **最小化合并阻力**：测试偶发失败用重跑处理而不是阻塞进度，在高吞吐环境下等待人工审查的成本往往高于修复小错误的成本。写代码的纪律没有消失，只是从人工 Review 变成了机器执行的约束，一次写进去，到处生效。

![Codex 可观测性栈](/images/others/agent-principles-architecture-and-engineering-practice/005-276d6af4.svg)
...
![Harness 关键结论](/images/others/agent-principles-architecture-and-engineering-practice/006-df2b4c3b.svg)
...
![上下文分层结构](/images/others/agent-principles-architecture-and-engineering-practice/007-b50e32ad.svg)
...
![Skills 按需加载](/images/others/agent-principles-architecture-and-engineering-practice/008-8f6a646b.webp)
...
![ACI 工具设计对比：差工具设计会让 Agent 反复绕圈，好工具设计能让 Agent 更快选对并修正错误](/images/others/agent-principles-architecture-and-engineering-practice/009-64f3f831.svg)
...
![四种记忆类型与存储位置：上下文窗口位于运行时 messages\[\]，Skills、JSONL 会话历史和 MEMORY.md 位于磁盘，生命周期和注入方式各不相同](/images/others/agent-principles-architecture-and-engineering-practice/010-229c01ca.svg)
...
![记忆整合与回退流程：消息流在 token 使用率超过阈值后触发整合，成功时摘要写入 MEMORY.md 并移动整合指针，失败时原始消息写入 archive/ 保留完整历史](/images/others/agent-principles-architecture-and-engineering-practice/011-c4001e0a.svg)
...
![Initializer + Coding Agent 跨 session 协作流程：Initializer 只运行一次并生成 feature-list.json、init.sh、初始 commit 和 claude-progress.txt，后续 Coding Agent 在多个 session 中通过文件系统恢复状态、实现单个功能、测试、更新 passes 并提交代码](/images/others/agent-principles-architecture-and-engineering-practice/012-725d5263.svg)
...
![AI 工作模式变化](/images/others/agent-principles-architecture-and-engineering-practice/013-d0737f12.svg)

常见的组织方式是主 Agent 作为 Orchestrator 统筹全局，下挂多个子 Agent 独立并行工作，它们之间通过 JSONL inbox 协议通信，用 Worktree 隔离文件修改，用任务图管理依赖关系。

![多 Agent 拓扑](/images/others/agent-principles-architecture-and-engineering-practice/014-791669c8.svg)
...
![多 Agent 协作协议](/images/others/agent-principles-architecture-and-engineering-practice/015-de207b57.svg)
...
![多 Agent 幻觉放大](/images/others/agent-principles-architecture-and-engineering-practice/016-6ae30d44.svg)
...
![Single-turn vs Agent 评测对比：Single-turn 是 Prompt 进 LLM 出 Response 直接打分，Agent 则需要 Tools、Environment、Task 协同，Agent 多步调用工具并更新环境状态，最后验证环境实际结果而非只看输出文字](/images/others/agent-principles-architecture-and-engineering-practice/017-7c5925ad.webp)
...
![Agent 评测的组成部分：task、trial、grader、transcript、outcome、evaluation harness、agent harness 和 evaluation suite](/images/others/agent-principles-architecture-and-engineering-practice/018-6b218304.webp)
...
|                                                                                                                                                                     |                                                                                                                                               |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| ![调查：团队实际使用的评测方式，Offline evaluation on test sets 54.5%，Online evaluation on production data 44.8%，Not evaluating yet 22.8%](/images/others/agent-principles-architecture-and-engineering-practice/019-5bb49264.webp) | ![调查：常用评测指标，Internal human review/labelling 59.8%，LLM-as-judge 53.3%，Traditional ML/DS metrics 16.9%](/images/others/agent-principles-architecture-and-engineering-practice/020-7973baad.webp) |
...
![Success rate vs infra error rate：横轴是评测容器的资源余量从 1x 到 Uncapped，蓝色是模型得分，红色是基础设施错误率，资源越受限红色越高蓝色越低](/images/others/agent-principles-architecture-and-engineering-practice/021-545cc76d.webp)
...
![两层可观测性](/images/others/agent-principles-architecture-and-engineering-practice/022-4e52da86.svg)
...
![事件流可观测性](/images/others/agent-principles-architecture-and-engineering-practice/023-e3070cfe.svg)
...
![OpenClaw 整体架构](/images/others/agent-principles-architecture-and-engineering-practice/024-dc0f9c0a.svg)
...
![系统提示分层叠加](/images/others/agent-principles-architecture-and-engineering-practice/025-7e6441d0.svg)
...
1. **安全边界先于功能**，工作空间隔离、白名单、参数验证，加任何新功能之前就要到位
1. **记忆整合要早做**，不加整合，第 20 轮对话之后基本就垮了
1. **Skills 先于新工具**，领域知识用文档管理，比加新工具更灵活
1. **第一个失败就建评测**，把第一个真实失败案例转成测试用例，不要等积累够了再开始
...
1. Harness，也就是验收基线、执行边界、反馈信号、回退手段，往往比模型本身更决定系统能否收敛，高质量自动化验证和清晰目标缺一不可。
1. 上下文工程的重点是防 Context Rot，通过分层管理常驻信息、按需知识、运行时信息和记忆，再配合滑动窗口、LLM 摘要、工具结果替换和 Skills 延迟加载，才能把信号质量稳定住。
1. 工具设计按 ACI 原则来做：面向 Agent 目标，不是面向底层 API，边界明确，参数防错，定义里直接给示例，调试时优先检查工具描述，而不是先怀疑模型能力。
1. 记忆可以分成工作记忆、程序性记忆、情景记忆和语义记忆，`MEMORY.md`、按需检索和可回退整合，是跨会话保持一致性的关键。
1. 长任务稳定运行靠的是状态外化，Initializer Agent 把任务变成文件系统状态，Coding Agent 循环可重入，进度通过文件传递，不依赖上下文窗口。
1. 多 Agent 要先有任务图和隔离边界再引入并行，协议先于协作，子 Agent 只回传摘要，搜索和调试细节留在自己的上下文里。
1. 评测上，Pass\@k 验证能力边界，Pass^k 保证上线质量，评测系统出问题先修评测再动 Agent，不要基于失真信号调整方向。
1. 可观测性上，Trace 是排查的前提，事件流做底座一次发布多路消费，人工标注校准 LLM 自动打分，两层要一起用。
1. OpenClaw 把前面这些原则放进了一个可运行系统里，真正让 Agent 跑稳，靠的不是更复杂的循环，而是消息解耦、状态外化、分层提示、记忆整合和安全边界这些工程细节。

## 参考资料

1. OpenAI, [Harness engineering: leveraging Codex in an agent-first world](https://openai.com/index/harness-engineering/)
1. Cloudflare, [How we rebuilt Next.js with AI in one week](https://blog.cloudflare.com/vinext/)
1. Simon Willison, [I ported JustHTML from Python to JavaScript with Codex CLI](https://simonwillison.net/2025/Dec/15/porting-justhtml/)
1. Anthropic, [Introducing Agent Skills](https://claude.com/blog/skills)
1. Anthropic, [Managing context on the Claude Developer Platform](https://claude.com/blog/context-management)
1. LangChain, [State of Agent Engineering](https://www.langchain.com/state-of-agent-engineering)
