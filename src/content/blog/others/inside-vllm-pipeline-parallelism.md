...
vLLM 里的 Pipeline Parallelism（PP）并不是一个“把层均匀切到多张卡上”的简单开关。真正落到推理系统里，它会同时牵动调度器、连续批处理（continuous batching）、KV cache、执行器拓扑，以及预填充（prefill）/ 解码（decode）两条完全不同的运行路径。

这篇文章想回答四个问题：vLLM 里的 PP 到底在解决什么问题，它在源码里是怎么落地的，为什么 stage 切分不能只看层数，以及它在真实部署里什么时候划算、什么时候只是“能跑起来”。

如果只记一条结论：**vLLM 的 PP 本质上是一套推理运行时机制，而不只是模型层切分功能。**
...
1. 在推理系统里，PP 首先是偏容量导向（capacity-oriented）的手段，用来让模型装下、让每卡留出更多空间给 KV cache；只有在特定条件下，它才会进一步变成偏性能导向（performance-oriented）的优化。
2. 推理 PP 的切分对象不是请求，而是模型层图；stage 的本质是“持有一段层与参数，并对边界激活做接力处理”的执行边界。
3. vLLM 当前的 PP 不是纯运行时（runtime）透明机制，模型必须显式暴露“跨 stage 传什么中间态、首尾段承担什么职责、哪些层该留在哪一段”这些结构知识。
4. vLLM 的层切分策略不是机械均分，而是显式考虑了首段 embedding、末段 `norm` / `lm_head` 这类天然不公平结构。
5. 因此“层数均分”不等于“负载均分”，而“模型能放下”也不等于“系统跑得划算”。
6. vLLM 中 PP 的主通信形态是 stage 边界激活传输；这与 TP 的层内集合通信（collective）本质不同，所以 PP / TP 的优劣会被硬件拓扑显著改变。
7. 对在线推理来说，PP 的关键指标不是单次前向（forward）多快，而是 TTFT、TPOT、稳态吞吐、KV cache 可容纳量与尾延迟的综合平衡。
8. prefill 阶段更容易把 PP 的通信与气泡成本摊薄；decode 阶段则更容易暴露 stage 不均衡、调度空泡和末段关键路径问题。
9. 训练里的 1F1B 心智模型不能直接搬到推理，因为推理没有反向传播（backward），而且 decode 每一步还有采样得到的 token（sampled token）反馈闭环。
10. vLLM 里的“PP 可并发多个 batch 波次”并不等于“PP 必然高吞吐”；如果 stage 最慢者很慢，更多波次只是让更多工作排队等它。
11. 在拓扑一般、无 NVLink、跨节点或模型层数与 GPU 数不整齐的场景里，PP 可能比 TP 更合理；在单机高速互连、极度重视单 token 延迟的场景里，TP 往往更直接。
12. vLLM 当前实现里，PP 的真实难点不在“怎么切层”，而在“怎么让调度器、连续批处理（continuous batching）、KV cache、执行器与 stage 边界协同工作”。
13. `SupportsPP`、`IntermediateTensors`、`make_empty_intermediate_tensors`、`PPMissingLayer` 这几组抽象，是理解 vLLM PP 实现的主线。
14. `get_pp_indices` 体现了 vLLM 对真实推理不公平性的工程判断：余数层优先避开末段，在合适情况下还尽量避开首段。
15. Llama 这一类模型最能体现首段/中段/末段职责不对称：首段偏输入，中段偏纯 block，末段偏收尾与输出。
16. vLLM 的 PP 设计已经明显是“推理系统特有的 PP”，而不是把训练框架的术语简单搬过来。
...
设模型总共有 `L` 个 transformer blocks，外加输入侧和输出侧特殊模块，可抽象为 `F = H_out o B_(L-1) o ... o B_1 o B_0 o H_in`。

PP 会把 block 序列切成 `P` 个连续分段，也就是 `Γ_0, Γ_1, ..., Γ_(P-1)`。

每个 stage `s` 都对应一个局部函数 `x_(s+1) = f_s(x_s; W_s, KV_s)`。
...
从理论上讲，stage 是一段连续层的执行单元。  
从工程上讲，stage 是一个真实 rank 或一组 TP ranks 的集合，它同时拥有：
...
推理里最容易被忽略的一点是：  
decode 不只是“再跑一次前向”，而是“前向 + sample + 状态反馈闭环”。

### 性能模型

#### 1. 统一符号与单 stage 服务时间

为了避免把 prefill / decode 混在一起，下面统一记相位 `phi in {prefill, decode}`。  
对第 `k` 个服务波次、stage `s`，实际服务时间可写成 `T_s^phi(k) = C_s^phi(k) + M_s^phi(k) + R_s^phi(k) - Omega_s^phi(k)`。

其中：

- `C_s^phi`：本 stage 的纯计算时间
- `M_s^phi = M_in,s^phi + M_out,s^phi`：边界激活接收/发送时间
- `R_s^phi`：运行时（runtime）杂项，如 launch、buffer copy、状态更新、末段 sample
- `Omega_s^phi`：计算与通信、通信与通信之间可被隐藏的重叠量

再把计算项拆开，就是 `C_s^phi(k) = C_blocks,s^phi(k) + C_special,s^phi(k)`。

其中：

- `C_blocks,s^phi`：本 stage 所含 transformer blocks 的主体计算
- `C_special,s^phi`：embedding、final norm、lm_head、多模态输入准备、sample 等首尾特殊逻辑

这个拆分比简单写成一个 `T_s` 更严格，因为它明确区分了：

- block 数量带来的主体负载
- 首尾 stage 的结构性不公平项
- 运行时开销与通信隐藏程度

#### 2. prefill 的计算量：由 token 总量和长度平方项共同决定

设第 `k` 个 prefill wave 里有 `B_k` 个请求，第 `i` 个请求 prompt 长度为 `n_i`。  
记：

- `N_k = sum_i n_i`：本 wave 的总 prompt token 数
- `Q_k = sum_i n_i^2`：长度平方和
- `H`：hidden size
- `l_s`：stage `s` 持有的 block 数

则一个半定量但更接近实际 batching 的近似式是 `C_blocks,s^prefill(k) ≈ l_s * (a1 * N_k * H^2 + a2 * Q_k * H)`。

其中：

- `a1 * N_k * H^2` 可以理解为 QKV 投影、输出投影、MLP 这类近似线性于 token 数、二次于 hidden size 的主体项
- `a2 * Q_k * H` 可以理解为 attention score / apply value 带来的长度平方项

如果 wave 内请求长度相近，`n_i ≈ n_bar`，则 `N_k ≈ B_k * n_bar`，`Q_k ≈ B_k * n_bar^2`，于是 `C_blocks,s^prefill(k) ≈ l_s * (a1 * B_k * n_bar * H^2 + a2 * B_k * n_bar^2 * H)`。
...
#### 3. decode 的计算量：不再有平方项，而是线性依赖活跃序列数与上下文长度

设第 `k` 个 decode step 有 `B_k` 条活跃序列，平均已生成上下文长度为 `t_bar,k`。  
由于每条序列在该 step 只新增 `1` 个 query token，而历史 KV 已缓存，因此可近似写成 `C_blocks,s^decode(k) ≈ l_s * (b1 * B_k * H^2 + b2 * B_k * t_bar,k * H)`。

其中：

- `b1 * B_k * H^2` 对应本步 token 的投影、MLP、层内固定形态计算
- `b2 * B_k * t_bar,k * H` 对应 query 与历史 KV 的交互，随上下文长度线性增长，而不是平方增长

因此 prefill 与 decode 的主导项本质不同：

- prefill 更像 `token-rich + length-squared`
- decode 更像 `step-small + context-linear + fixed-overhead-visible`
...
设 stage 边界需要传输 `m_s` 份主激活张量，元素字节数为 `b`，链路带宽为 `BW_s`，固定时延为 `alpha_s`。  
记该相位下真正要跨 stage 传输的激活 token 数为 `n_act^phi(k)`，则 `A_s^phi(k) ≈ m_s * n_act^phi(k) * H * b`，同时 `M_out,s^phi(k) ≈ alpha_s + A_s^phi(k) / BW_s`。

对 decoder-only 模型，常见近似是：

- `n_act^prefill(k) = N_k`
- `n_act^decode(k) = B_k`

如果边界上传的是两份主激活，例如 `hidden_states` 与 `residual`，即 `m_s = 2`，并取 `H = 4096`、`b = 2` bytes，那么：

- prefill 若 `N_k = 4096`，则 `A_s^prefill ≈ 2 * 4096 * 4096 * 2 ≈ 64 MiB`
- decode 若 `B_k = 32`，则 `A_s^decode ≈ 2 * 32 * 4096 * 2 ≈ 512 KiB`

decode 的边界数据量看起来更小，但这不意味着 decode 一定更容易隐藏通信，因为同时发生的还有两件事：

- `C_s^decode` 比 `C_s^prefill` 小得多
- `alpha_s`、launch、sample 等固定成本没有按同样比例缩小

因此更有判断价值的是通信占比，也就是 `rho_comm,s^phi(k) = M_s^phi(k) / T_s^phi(k)`。

通常：

- prefill 更像带宽项 `A / BW` 主导
- decode 更容易让固定时延 `alpha` 和运行时杂项显形

#### 5. 从单 stage 推到整条 pipeline：cycle time、bubble 与利用率

设共有 `P` 个 stage，定义该相位下的稳态节拍为 `tau^phi = max_s E[T_s^phi]`，也就是最慢 stage 的平均服务时间。  
再定义平均 stage 时间与不均衡系数：`T_bar^phi = (1 / P) * sum_s E[T_s^phi]`，`gamma^phi = tau^phi / T_bar^phi >= 1`。

其中：

- `gamma^phi = 1` 表示完全均衡
- `gamma^phi > 1` 表示存在最慢 stage 拖尾

若同一相位中能让 `W` 个服务波次重叠在 pipeline 中，则总完成时间可近似写成 `T_total^phi(W) ≈ sum_s E[T_s^phi] + (W - 1) * tau^phi`。

这比简单写成 `(P + W - 1) * T_stage` 更一般，因为它允许 stage 异质。  
把上式写成 `T_bar^phi` 和 `gamma^phi` 的形式，就是 `T_total^phi(W) ≈ P * T_bar^phi + (W - 1) * gamma^phi * T_bar^phi`。

于是平均设备利用率可近似为 `U^phi(W) ≈ (W * sum_s E[T_s^phi]) / (P * T_total^phi(W))`，进一步代入后得到 `U^phi(W) ≈ W / (P + (W - 1) * gamma^phi)`。

这个式子比原先的同质近似更有解释力，因为它同时显式刻画了两类 bubble：

- fill / drain bubble：`W` 太小时，由 `P` 这一项主导
- imbalance bubble：`gamma^phi > 1` 时，即使 `W` 很大，也会被最慢 stage 限制

特别地：

- 若 `gamma^phi = 1`，退化为熟悉的 `U^phi(W) = W / (P + W - 1)`
- 当 `W -> inf` 时，`U^phi(W) -> 1 / gamma^phi`

最后这条非常重要，它说明：  
**只靠增加并发波次并不能无限补救 stage 不均衡；如果末段长期更慢，最大可达利用率本身就被锁死了。**

#### 6. 吞吐、TTFT、TPOT 如何从这个模型里导出

稳态波次吞吐近似为 `Throughput_wave^phi ≈ 1 / tau^phi`。

若换成 token 吞吐，则还要乘上每个 wave 真正携带的 token 数：

- `Throughput_tok^prefill ≈ E[N_k] / tau^prefill`
- `Throughput_tok^decode ≈ E[B_k] / tau^decode`

对服务指标，可以进一步写成 `TTFT ≈ T_queue + sum_s T_s^prefill` 与 `TPOT ≈ tau^decode + T_fb - Omega_fb`。

其中：

- `T_queue`：请求进入 prefill 前的排队与 admission 等待
- `T_last^prefill` 内部已经包含末段产出首个 token 前的 final norm / lm_head / sample 开销
- `T_fb`：sampled token 回到下一轮调度、并重新分发到 decode step 的反馈时间
- `Omega_fb`：这部分反馈中可与其他工作重叠的量

于是一个生成 `m` 个输出 token 的请求，其端到端延迟可近似写成 `Latency_req ≈ TTFT + m * TPOT`。
...
更关键的是，decode 还多了训练里没有的闭环依赖，也就是 `Delta_decode(k) >= max(tau^decode(k), T_fb(k))`。

这表示第 `k + 1` 个 decode step 的真正起点，不只取决于 stage 是否空出来，还取决于：
...
把上面的量综合起来，PP 更可能“值得”的条件可以半定量地写成：

- `U^phi(W)` 不能太低，也就是要么 `W` 足够大，要么 `P` 不要切得过深
- `gamma^phi` 要尽量接近 `1`，否则最慢 stage 会把稳态上限锁在 `1 / gamma^phi`
- `rho_comm,s^phi` 不能长期过高，否则 stage 边界会吞掉算力收益
- `T_fb` 不能显著大于 `tau^decode`，否则 decode 会被反馈闭环而不是被 pipeline 节拍主导
- `max_s Mem_s <= Mem_gpu` 必须成立，否则讨论吞吐前系统先不具备可运行性

相反，PP 更可能只解决“能不能跑”，而不一定解决“跑得划不划算”的典型情形是：

- `W` 小，fill / drain bubble 很重
- `gamma^decode` 大，末段或首段长期偏慢
- `rho_comm^decode` 高，decode 每步通信与固定时延难以隐藏
- `T_fb` 显著，sample 与 token 回传进入关键路径

从这个角度看，PP 的性能判断不是一句“层均分了没有”，而是 `{U, gamma, rho_comm, T_fb}` 这四个量的联立判断。

这四个量基本就对应了推理 PP 最核心的四个现实问题：
...
如果只看 block 权重，PP 后每卡权重大体接近按 stage 数分摊。  
但真实情况不是简单 `1 / P`，因为还有结构性不公平项：
...
所以更准确的表达是 `Mem_weight,s = Mem_blocks,s + Mem_special,s`。
...
### 1. 推理 PP 的核心不是切层，而是运行时

按层切分只解释了静态图，不解释运行时。  
真正决定效果的是：

- wave 能不能把流水线灌满
- decode 的 sampled token 怎么反馈
- scheduler 怎么处理动态批形
- KV cache 怎么按 stage 分布

懂层切分（layer split）只是入门，懂运行时（runtime）才算真正懂推理 PP。

### 2. PP 常常先解决部署，再谈性能

对推理服务，显存和拓扑通常先于算力成为约束。  
模型装不下时，PP 的第一价值是让单副本可运行；只有在通信便宜、stage 均衡、流水线够饱和时，它才会进一步转化成吞吐收益。

### 3. 连续批处理、decode 与拓扑共同决定 PP 上限

训练中的固定 micro-batch 心智不适用于推理。  
连续批处理（continuous batching）让 batch 是动态的，decode 又放大了边界通信、sample、token 反馈和状态更新；于是 PP 的收益往往取决于最慢 stage 和链路质量，而不只是模型大小。

### 4. 服务决策看的是 TTFT、TPOT、吞吐与显存的联立

- TTFT：更受 prefill 全链路与初始 bubble 影响
- TPOT：更受 decode 最慢 stage 与末段反馈影响
- 吞吐：更依赖 pipeline 填充程度与 stage 均衡
- 显存利用率：PP 常常先受益，因为权重更薄

判断 PP 值不值得，本质上是在这些目标之间做取舍（trade-off）。

### 5. vLLM 的 PP 是“推理系统特有”的实现

vLLM 并不是把训练框架术语直接搬来，而是围绕推理场景做了几件关键设计：

- 以 `IntermediateTensors` 作为 stage 边界协议
- 以 `make_empty_intermediate_tensors` 适配 profiling / 图捕获
- 允许多个 batch waves 重叠以减少 bubble
- 在 decode 中显式处理 sampled token 反馈
- 让 KV cache 配置与 stage 归属联动
