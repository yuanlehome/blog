...
### 工程实现解读：Loss Mask 如何实现多轮对话的轨迹拆分
...
### 工程实现解读：Ratio 计算与 PPO Clip 的代码实现
...
### 工程实现解读：Entropy 的计算与 Entropy Bonus
...
### 工程实现解读：OPSM——Off-Policy Sequence Masking
...
### 工程实现解读：slime 中 Routing Replay 的完整实现
...
### 工程实现解读：slime 的 RolloutBuffer 设计
...
一条回答、一条 reasoning chain、一次 agent 多轮交互，本质上都是轨迹。

### 2. 轨迹拆分是为了 credit assignment

你需要决定：最终 reward 应该如何分配到 token、step、turn 上。

### 3. Reward 告诉系统"什么是好"

但真正进入优化时，通常会先变成 return / advantage，而不是直接拿裸 reward 做梯度。

### 4. IS 解决"旧样本如何服务当前策略"

概率比（ratio）是 RL loss 的核心稳定性变量之一。

### 5. KL 解决"别跑太远"

K3 通常只是 sampled KL 的一种 estimator / surrogate，不应和 KL 本体混为一谈。

### 6. Entropy 解决"别太早失去探索"

过早熵塌缩会让策略陷入局部模式，尤其影响 reasoning RL。

### 7. Loss 是多个目标的组合

Policy、Value、KL、Entropy 常常共同构成最终训练目标。

### 8. 训推一致是 RL Infra 的关键工程课题

在 LLM RL 中，真正让训练变脏的，往往不是公式本身，而是 rollout 和 trainer 之间的不一致。

### 9. KSM 修 action space，一致地算 token 概率

training 侧不重新采样，但要在 rollout 的同一 sampling support 上重算 logprob。

### 10. R3 / routing replay 修 parameter subspace，一致地走 expert 路径

特别是在 MoE 模型中，trainer 不仅要复现 token 概率空间，还要尽量复现 rollout 的 expert 路径。
