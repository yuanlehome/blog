...
[![LinkedIn](/images/others/slime/001-d1241118.svg) ](https://www.linkedin.com/in/biao-he/)[![X](/images/others/slime/002-b0e70309.svg) ](https://x.com/hebiao064)[![GitHub](/images/others/slime/003-09285634.svg)](https://github.com/hebiao064)

<!-- Author 2 -->

### [朱子霖](#)

[![LinkedIn](/images/others/slime/001-d1241118.svg) ](https://www.linkedin.com/in/zilin-zhu/)[![GitHub](/images/others/slime/003-09285634.svg)](https://github.com/zhuzilin)

<!-- Author 3 -->

### [李冀](#)

[![LinkedIn](/images/others/slime/001-d1241118.svg) ](https://www.linkedin.com/in/ji-li-a4892623b/)[![GitHub](/images/others/slime/003-09285634.svg)](https://github.com/GeLee-Q)
...
- **易维护** - 轻量级代码库，并可从Megatron预训练平滑过渡到SGLang部署。[1](#fn:1)
...
1. **推理引擎**生成rollout、采样动作，但它需要使用**最新的策略模型权重**以与训练保持一致。
1. 这两个组件通常分别运行在不同的进程和不同的框架（如Megatron/FSDP vs. SGLang/vLLM），因此**需要显式同步**。
...
1. **序列化为CUDA IPC**：将Tensor转换为CudaIpcHandlers并将其聚合成一个个大约为512MB的bucket tensor中。[代码](https://github.com/THUDM/slime/blob/e943681211e2b230f2a34efd9793e1257c2d70c7/slime/backends/megatron_utils/update_weight_utils.py#L402-L416)
1. **API通信**：通过`update_weights_from_tensor` api将序列化好的CudaIpcHandlers发送到SGLang Server。[代码](https://github.com/THUDM/slime/blob/main/slime/backends/sglang_utils/sglang_engine.py#L151-L171)
1. **分发到工作节点**：SGLang Server将CudaIpcHandlers分发到SGLang在各个GPU Rank上启动好的TP Worker进程。[代码](https://github.com/sgl-project/sglang/blob/5343058875a7c07ad62cfef9681f26ffbe359859/python/sglang/srt/managers/tokenizer_manager.py#L1153-L1155)
1. **重构和加载**：TP Worker将CudaIpcHandlers反序列化并进行映射，指向Megatron之前聚合好的同一片GPU地址，从而将Megatron的权重加载到SGLang中。[代码](https://github.com/sgl-project/sglang/blob/v0.5.1/python/sglang/srt/model_executor/model_runner.py#L971)
...
## [4. 我们的工作：将QWen3-30B-A3B模型的权重更新时间从60秒优化到7秒](#4-qwen3-30b-a3b607)

![我们的优化之旅](/images/others/slime/007-cc811f49.png)

> **注意**：上图是根据这个Github Issue[3](#fn:3)里的所有PR做完之后往回捋出来的，以便更容易理解逻辑，实际上，我们没有按照上图所示的改进顺序进行，因为实际工作场景中自然是按照从易到难实现，而不是根据物理传输过程中的顺序。

### [4.0 GPU上的跨进程数据传输：CUDA IPC Handler](#40-gpucuda-ipc-handler)
...
#### [利用CUDA IPC Handler同卡零拷贝传输](#cuda-ipc-handler)
...
1. **最小CPU内存开销**：CUDA IPC Handler非常小 vs 序列化数据的GB级别

这其实只是我们的baseline实现，虽然比直接传数据要快得多，但仍然花了60秒，显然有很多优化空间。

### [4.1 优化Megatron Worker中Tensor聚合过程：_从60秒到50秒_](#41-megatron-workertensor6050)
...
### [4.2 通过Tensor分桶优化SGLang服务器调用：_从50秒到30秒_](#42-tensorsglang5030)
...
### [4.3 合并多个Tensor成一个Tensor：减少CUDA IPC开销：_从30秒到20秒_](#43-tensortensorcuda-ipc3020)
...
### [4.4 加载权重优化：最终性能提升：_从20秒到7秒_](#44-207)
...
1. [slime: An SGLang-Native Post-Training Framework for RL Scaling](https://lmsys.org/blog/2025-07-09-slime/) [↩](#fnref:1)

1. [GLM-4.5: Agentic, Reasoning, and Coding (ARC) Foundation Models](https://arxiv.org/pdf/2508.06471) [↩](#fnref:2)

1. [slime Issue #132: Weight Sync Optimization in Colocate Mode](https://github.com/THUDM/slime/issues/132) [↩](#fnref:3)
