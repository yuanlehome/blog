...
全局内存访问的主要考虑因素称为合并（coalescing），一句话总结是：当相邻线程访问全局内存中的相邻数据时，可实现最大全局内存带宽（解释[此处](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#device-memory-accesses)）。共享内存将在[后续](#背景 bank 冲突和 wavefronts)章节中深入探讨。
