...
此图中的红色虚线被称为硬件的"平衡点"（balance point），它是我们需要超越的算术强度水平（以FLOP/字节为单位），以便从内存受限变为计算受限。如果我们将此值称为I∗，则I∗∗β=τ或等价地I∗=τβ。它是特定计算机的属性，即峰值浮点吞吐量除以内存带宽。由于摩尔定律，算术吞吐量的改进速度远快于内存带宽，其结果是一般来说，计算机越新，平衡点就越高。

## NVIDIA Tesla T4 的屋顶线

代入我们使用的GPU的具体数字，并查看生成的屋顶线可以指导我们的算法设计，并让我们对即将面临的情况有所了解。在真实计算机上，不仅仅有单一的τ和β，有多个硬件指令，每个都有不同的峰值吞吐量τ，以及不同类型的内存，每个都有不同的带宽β。
...
根据NSight Compute，Kernel 4执行的总指令中有92%位于循环嵌套中，其中每个warp从共享内存加载其数据区域到寄存器内存，然后通过一系列`HMMA`指令对存储在寄存器内存中的本地矩阵执行外积。将`HMMA`指令映射到其位置的三个嵌套循环都完全展开，因此那里不需要任何运行时索引计算。
...
优化的索引计算、循环展开和调整后的瓦片维度都作为同一内核的一部分实现，相比前一个内核实现了来之不易的1.2倍加速，使我们达到cuBLAS吞吐量的86.7%。 ![table5](/images/others/how-to-write-a-fast-matrix-multiplication-from-scratch-with-tensor-cores/052-4477720d.png)

## 内核 6 - 双缓冲

回到分析器（最后一次）。此时，我的内核和cuBLAS之间的许多指标开始看起来有些相似。让我注意到的一点是，我的内核中的线程在`__syncthreads()`上停滞的时间比cuBLAS内核更多。此时我的内核的CPI（每指令周期数）为14，其中约2.6个周期来自同步停滞。因此这不是一个严重的性能问题，但很明显。一种称为双缓冲的技术使你能够从内循环中移除两个`__syncthreads()`中的一个。经过一番思考后，我意识到这并不保证在`__syncthreads()`上停滞的周期会成比例减少（如果你移除一个`__syncthreads()`，线程可能会在另一个上花费两倍的停滞时间）。然而，双缓冲也应该允许主循环内部有更多的指令级并行性，并且它在CUTLASS内核中实现，而且我有共享内存可用，所以为什么不呢。
