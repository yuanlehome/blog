...
图中的红色虚线被称为硬件的"平衡点"，它是我们需要超越的算术强度水平（以FLOP/字节为单位），以便从内存受限转变为计算受限。如果我们将这个值称为I∗，那么I∗∗β=τ，或等价地I∗=τ/β。它是特定计算机的属性，即峰值浮点吞吐量除以内存带宽。由于摩尔定律，算术吞吐量的改进速度远快于内存带宽，其结果是，一般来说，计算机越新，平衡点就越高。

## NVIDIA Tesla T4的屋顶线

插入我们使用的GPU的一些具体数字，并查看得到的屋顶线图，可以为我们的算法设计提供信息，并让我们对即将面临的挑战有所了解。在真实的计算机上，不只有一个τ和β，而是有多个硬件指令，每个都有不同的峰值吞吐量τ，以及不同类型的内存，每个都有不同的带宽β。

### 张量核心 vs. FFMA

首先，我们需要知道我们设备的全局内存带宽βgmem。NVIDIA规格表报告*理论*内存带宽，这在实践中[永远无法实现](https://forums.developer.nvidia.com/t/theoretical-bandwidth-vs-effective-bandwidth/48005/3?u=a14armbr)。实际数字可以通过基准测试找到，根据[这篇白皮书](https://arxiv.org/pdf/1903.07486)，T4的可实现内存带宽为220 GB/秒（这是320 GB/秒理论内存带宽的68%）。接下来，我们需要知道使用张量核心的峰值浮点吞吐量，以及不使用它的峰值浮点吞吐量。类似于内存，理论数字[实际上无法实现](https://www.thonking.ai/p/strangely-matrix-multiplications)而无需GPU着火或熔化。我认为使用cuBLAS半精度（使用张量核心）和单精度（不使用张量核心）GEMM内核的实测吞吐量作为可实现的浮点吞吐量数字是合理的。查看cuBLAS半精度内核的汇编代码，我们可以看到繁重的工作是由`HMMA.1688`完成的，该指令执行一次小型硬件加速矩阵乘法（稍后详述）。对于单精度GEMM内核，执行工作的指令称为`FFMA`，这是一个标量乘加操作，d=a∗b+c。根据我的基准测试，张量核心HMMA.1688的吞吐量为49439 GFLOP/秒，我们称之为τHMMA。非张量核心FFMA的吞吐量为7455 GFLOP/秒，我们称之为τFFMA。这些分别是理论峰值吞吐量的76%和92%，看起来足够合理。由此得到的屋顶线图如下所示（这些图通常以对数/对数比例显示，但此图不是）：
...
张量核心可通过两种不同方法访问。第一种是通过`wmma` [API](https://docs.nvidia.com/cuda/cuda-c-programming-guide/#wmma-description)，它是CUDA工具包的一部分。`wmma`似乎被认为是更可移植但性能较低的方式来编程张量核心。我很快放弃了它，因为它抽象了将输入数据从共享内存加载到寄存器内存的过程，而事实证明这里的一些细节对性能至关重要。

另一种途径是使用`mma`指令家族，它们是PTX的一部分，此选项比`wmma`途径更灵活且性能更高。PTX是NVIDIA GPU的中间表示，比CUDA低级，但比SASS（这是NVIDIA GPU运行的汇编语言）高级。PTX可以内联在内核中以调用张量核心。
...
最让我注意的一件事是，cuBLAS HGEMM执行的指令总数为94,175,232条，而Kernel 4执行了216,227,840条指令，是Kernel 4的两倍多。虽然Kernel 4通过较低的每条指令周期数比率（约8，而cuBLAS约12）部分地弥补了这一点，但这确实值得研究。

所以我想，为什么我的内核执行的指令数量是两倍？在NSight Compute中展开指令混合部分可以获得更多信息。![instruction_mix_comparison](/images/others/how-to-write-a-fast-matrix-multiplication-from-scratch-with-tensor-cores/047-0086fb98.png) 答案是Kernel 4执行的索引计算相关指令比cuBLAS内核多得多。`LOP`、`IADD3`和`SHF`指令是整数和逻辑指令，它们与张量核心使用不同的流水线，可以与芯片上其他地方发生的浮点数学运算并发执行。然而，流式多处理器上的每个warp调度器每个周期只能发出一条指令，因此大量的索引计算指令可能会挤占`HMMA`指令的发出，而这些才是执行繁重工作的张量核心指令。那么这些整数和逻辑指令在做什么，为什么有这么多？

根据NSight Compute，Kernel 4执行的总指令的92%位于循环嵌套中，其中每个warp将其数据区域从共享内存加载到寄存器内存中，然后使用一系列`HMMA`指令对存储在寄存器内存中的局部矩阵执行外积。将`HMMA`指令映射到其位置的三个嵌套循环都完全展开了，因此在那里不需要任何运行时索引计算。

然而，`HMMA`指令操作存储在寄存器中的8×8分块，在计算阶段之前，每个warp中的线程协同工作，使用`ldmatrix` PTX指令将所有这些分块从交织的共享内存加载到寄存器内存中（有关`ldmatrix`的解释，请参见[这里](#如何使用张量核心)）。由于此时我们已经到达了分块层次结构的最底层，分块非常小，因此我们要执行*很多*次这种索引计算（O(N³/8)），它涉及乘以一堆步长、计算关于线程索引的模运算以及几个逻辑操作来应用交织函数，所有这些都在运行时发生。
...
为了使其更高效，我们应该将尽可能多的计算移到编译时发生，而需要在运行时发生的任何计算都应该尽可能精简。在上面显示的索引计算代码中，从根本上说有三个不同且相互依赖的步骤：

1. 首先，每个warp计算MMA分块左上角的内存地址
1. 每个线程计算它将加载的元素的内存地址，相对于(1)
1. 因为我们的共享内存布局是交织的，每个线程将交织函数应用于在(2)中计算的地址，以获得交织布局中的正确内存地址。

对于每个8×8 MMA分块都要执行这三个步骤。下面是对此的可视化，下图是一个小例子，其中每个MMA分块有四行一列，每个warp分块有2×8个MMA分块（使用这样更简单的例子可以让我们尽可能明确所有细节，而魔鬼就在细节中）。
...
在中间列中，每个线程已经计算了它将要加载的值的地址（在未交织的布局中）。每次迭代，这些指针向右前进一列，直到到达warp分块的末尾，此时我们向下移动到下一组行。如果不是交织布局，我们可以在每次迭代中将指针前进一个，即`thread_row+=1`。然而，由于数据以交织布局存储，将指针前进到下一组MMA分块并不是简单地递增一的问题。

虽然递增一不适用于遍历交织布局，但我们可以通过将每个线程的指针与一个常数进行异或（XOR）来实现等效效果。![swizzled_index_calculation_efficient](/images/others/how-to-write-a-fast-matrix-multiplication-from-scratch-with-tensor-cores/051-3f9c84d9.png) 这将每个`ldmatrix`之间的索引计算量从约13个操作减少到单个XOR。应用此优化后，执行的指令总数下降到约90M，略少于cuBLAS。

这说明了高效遍历交织数据布局的基本原则。在[实际代码](https://github.com/alexarmbr/matmul-playground/blob/main/src/kernel5.cu#L10)中，它更复杂一些，因为交织函数更复杂，并且我们需要遍历A和B的分块，它们彼此具有不同的维度。此外，包含`ldmatrix`指令的循环是手动展开的，这使得异或更容易，也可能允许编译器更好地交错`ldmatrix`和`mma.sync`指令，以平衡两个不同流水线之间的负载。

优化的索引计算、循环展开和调整的分块维度都作为同一内核的一部分实现，实现了相对于上一个内核来之不易的1.2倍加速，并使我们达到了cuBLAS吞吐量的86.7%。![table5](/images/others/how-to-write-a-fast-matrix-multiplication-from-scratch-with-tensor-cores/052-4477720d.png)

## 内核6 - 双缓冲

回到分析器（最后一次）。此时，我的内核和cuBLAS之间的许多指标开始看起来有些相似。最让我注意的一件事是，我的内核中的线程在`__syncthreads()`上停滞的时间比cuBLAS内核更长。此时我的内核的CPI（每条指令周期数）为14，其中约2.6个周期来自同步停滞。所以这不是一个严重的性能问题，但很明显。一种称为双缓冲的技术使你能够从内循环中删除两个`__syncthreads()`中的一个。经过一番思考，我意识到这不能保证停滞在`__syncthreads()`上的周期成比例减少（如果你删除一个`__syncthreads()`，线程可能会在另一个上花费两倍的停滞时间）。然而，双缓冲也应该允许主循环内部有更多的指令级并行性，并且它在CUTLASS内核中实现，而且我有共享内存可用，所以为什么不呢。
...
双缓冲背后的想法是，如果我们为A和B的块分块分配额外的共享内存缓冲区对，我们可以写入一对缓冲区，同时另一对被读取。这允许我们从主循环中移除第二个`__syncthreads()`。这应该会使事情变得更快一些。
