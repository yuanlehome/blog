...
- 第一个 `qkv inear` 层的权重 $A$ 被按_列_切分：$A = [A_1, A_2, \dots, A_N]$。
...
1. **数据排布**：
    ...
    - **权重 (**$W$**)**：`out_linear` 权重 $W$ 必须_同样_按 $H_{\text{dim}}$ 维度（即_行_）切分：
$$
...
- **缺点**：
    - **通信瓶颈**：必须在计算 $Z_i$ _之后_执行一个 `all_reduce`。这是一个同步操作，通信量为 $Z$ 的大小，可能会阻塞流水线。
