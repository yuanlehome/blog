...
import { serializeError } from '../../scripts/utils/errors.js';
...
describe('Error Serialization', () => {
  it('should serialize Error objects with message, name, and stack', () => {
    ...
    expect(serialized.stack).toContain('TestError');
  });
  ...
  it('should serialize Error with cause', () => {
    ...
    const serialized = serializeError(mainError);
  ...
  it('should serialize null and undefined', () => {
    ...
    expect(nullResult.name).toBe('UnknownError');

    const undefinedResult = serializeError(undefined);
    expect(undefinedResult.message).toBe('undefined');
    expect(undefinedResult.name).toBe('UnknownError');
  });

  it('should serialize plain objects with message', () => {
    ...
    expect((serialized as any).code).toBe(500);
  });

  it('should serialize primitive types', () => {
    const stringResult = serializeError('Error string');
    expect(stringResult.message).toBe('Error string');
    expect(stringResult.name).toBe('UnknownError');

    const numberResult = serializeError(404);
    expect(numberResult.message).toBe('404');
    expect(numberResult.name).toBe('UnknownError');
  });

  it('should redact sensitive information in error properties', () => {
    const error = new Error('Request failed') as any;
    error.details = 'api_key=secret-key-12345';
    error.authorization = 'Bearer abc123def456';
  ...
  it('should not produce empty error objects', () => {
    const error = new Error('Test error');
