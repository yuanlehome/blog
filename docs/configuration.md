...
**⭐ 新增**：页眉中的动态线条小人儿，为博客添加生动有趣的互动元素。

#### headerMascot

| 字段           | 类型    | 默认值 | 说明                                      |
| -------------- | ------- | ------ | ----------------------------------------- |
| `enabled`      | boolean | true   | 是否启用页眉小人儿                        |
| `speed`        | number  | 1.0    | 行走速度倍数（0.1-5.0，数值越大速度越快） |
| `interactive`  | boolean | true   | 是否启用交互功能（悬停/点击）             |
| `hideOnMobile` | boolean | true   | 在移动设备上是否隐藏                      |
...
2. **交互功能**（当 `interactive: true` 时）
   - **悬停**：小人停下并挥手
   - **点击**：小人跳跃

3. **可访问性**
   - 自动遵循 `prefers-reduced-motion` 设置
   - 设置了 `aria-hidden="true"`（非信息性装饰元素）
   - 不阻挡导航点击
...
- SVG 图形轻量（< 1KB）
- 不引入任何第三方库
- 遵循 `prefers-reduced-motion` 时自动禁用动画
